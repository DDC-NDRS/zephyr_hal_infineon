...
static CEDI_OBJ const* cyp_ethif_gemgxlobj;

/** Ethernet configurations */
CY_NOINIT static CEDI_Config cy_ethif_cfg[CY_ETH_DEFINE_NUM_IP];
...
static void* cyp_ethif_pd[CY_ETH_DEFINE_NUM_IP];
...
static cy_en_ethif_status_t Cy_ETHIF_WrapperConfig(uint8_t u8EthIfInstance,
...
static cy_en_syspm_status_t Cy_ETHIF_DSCallbackFunc(cy_stc_syspm_callback_params_t* callbackParams,
...
static CEDI_Callbacks const Cy_ETHIF_Callbacks = {
...
cy_en_ethif_status_t Cy_ETHIF_Init(ETH_Type* base, cy_stc_ethif_mac_config_t* pstcEthIfConfig,
    ...
    uint32_t u32RetValue;
    uint8_t u8EthIfInstance;
    uint16_t u16SysReqTxBDLength;
    uint16_t u16SysReqRxBDLength;
    ...
    u16SysReqTxBDLength = (uint16_t)((cy_ethif_sysreq.txDescListSize / CY_ETH_DEFINE_NUM_TXQS) /
                                     (CY_ETH_DEFINE_TOTAL_BD_PER_TXQUEUE + 1U));
    u16SysReqRxBDLength = (uint16_t)((cy_ethif_sysreq.rxDescListSize / CY_ETH_DEFINE_NUM_RXQS) /
                                     (CY_ETH_DEFINE_TOTAL_BD_PER_RXQUEUE + 1U));
    ...
    for (size_t tmpcounter = 0; tmpcounter < cy_ethif_cfg[u8EthIfInstance].rxQs; tmpcounter++) {
        if (pstcEthIfConfig->pRxQbuffPool[tmpcounter] != NULL) {
            g_pRxQbuffPool[u8EthIfInstance][tmpcounter] = pstcEthIfConfig->pRxQbuffPool[tmpcounter];

            for (size_t tmpintrcntr = 0; tmpintrcntr < cy_ethif_cfg[u8EthIfInstance].rxQLen[tmpcounter];
                 tmpintrcntr++) {
    ...
    cy_stc_syspm_callback_t* cb = &g_ds_cbHandler[u8EthIfInstance];

    cb->callback       = Cy_ETHIF_DSCallbackFunc;
    cb->type           = CY_SYSPM_DEEPSLEEP;
    cb->skipMode       = 0;
    cb->callbackParams = &g_ds_params[u8EthIfInstance];
    cb->prevItm        = NULL;
    cb->nextItm        = NULL;
    cb->order          = 0;
...
*
*******************************************************************************/
cy_en_ethif_status_t Cy_ETHIF_TransmitFrame(ETH_Type* base, uint8_t* pu8TxBuffer, uint16_t u16Length,
                                            uint8_t u8QueueIndex, bool bEndBuffer) {
    uint8_t u8EthIfInstance;
    uint8_t u8flags = 0;
    uint32_t u32result;
...
void Cy_ETHIF_SetNoBroadCast(ETH_Type* base, bool rejectBC) {
    ...
    if (!CY_ETHIF_IS_IP_INSTANCE_VALID(base)) {
        ...
        cyp_ethif_gemgxlobj->setNoBroadcast(cyp_ethif_pd[u8EthIfInstance], rejectBC);
    }
}
...
void Cy_ETHIF_SetPromiscuousMode(ETH_Type* base, bool toBeEnabled) {
    ...
    if (!CY_ETHIF_IS_IP_INSTANCE_VALID(base)) {
        ...
        cyp_ethif_gemgxlobj->setCopyAllFrames(cyp_ethif_pd[u8EthIfInstance], toBeEnabled);
    }
}
...
*
*******************************************************************************/
cy_en_ethif_status_t Cy_ETHIF_SetFilterAddress(ETH_Type* base, cy_en_ethif_filter_num_t filterNo,
                                               cy_stc_ethif_filter_config_t const* config) {
    uint8_t u8EthIfInstance;
    ...
    (void)cyp_ethif_gemgxlobj->setSpecificAddr(cyp_ethif_pd[u8EthIfInstance],
                                               filterNo,
...
cy_en_ethif_status_t Cy_ETHIF_Get1588TimerValue(ETH_Type* base, cy_stc_ethif_1588_timer_val_t* stcRetTmrValue) {
    ...
    (void) cyp_ethif_gemgxlobj->get1588Timer(cyp_ethif_pd[u8EthIfInstance], (CEDI_1588TimerVal*)stcRetTmrValue);
...
cy_en_ethif_status_t Cy_ETHIF_Set1588TimerValue(ETH_Type* base, cy_stc_ethif_1588_timer_val_t* pstcTmrValue) {
    ...
    CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 11.3', 'Intentional typecast of stcRetTmrValue to CEDI_1588TimerVal* struct type.');
    if (((uint32_t)EOK) !=
        cyp_ethif_gemgxlobj->set1588Timer(cyp_ethif_pd[u8EthIfInstance], (CEDI_1588TimerVal*)pstcTmrValue)) {
...
void Cy_ETHIF_DecodeEvent(ETH_Type* base) {
    ...
    if (!CY_ETHIF_IS_IP_INSTANCE_VALID(base)) {
        ...
        (void) cyp_ethif_gemgxlobj->isr(cyp_ethif_pd[u8EthIfInstance]);
    }
}
...
void Cy_ETHIF_DiscardNonVLANFrames(ETH_Type* base, bool enable) {
    ...
    if (!CY_ETHIF_IS_IP_INSTANCE_VALID(base)) {
        ...
        cyp_ethif_gemgxlobj->setVlanOnly(cyp_ethif_pd[u8EthIfInstance], (uint8_t)(enable ? 1 : 0));
    }
}
...
static void Cy_ETHIF_PrepareConfiguration(uint8_t u8EthIfInstance, cy_stc_ethif_mac_config_t* pstcEthIfConfig) {
    ...
    CEDI_Config* cfg = &cy_ethif_cfg[u8EthIfInstance];

    /** Clear configuration table   */
    (void) memset((void*)cfg, 0, sizeof(cy_ethif_cfg[u8EthIfInstance]));

    /** Load GEM_GXL register base address  */
    cfg->regBase = CY_ETHIF_GEMGXL_ADDR_REGBASE(u8EthIfInstance);
    // (u8EthIfInstance == CY_ETHIF_INSTANCE_0) ? CY_ETH0_GEMGXL_ADDR_REGBASE : CY_ETH1_GEMGXL_ADDR_REGBASE;

    /** Prepare Queues  */
    cfg->rxQs = CY_ETH_DEFINE_NUM_RXQS;
    cfg->txQs = CY_ETH_DEFINE_NUM_TXQS;

    for (size_t queueCounter = 0; queueCounter < cfg->rxQs; queueCounter++) {
        cfg->rxQLen[queueCounter] = CY_ETH_DEFINE_TOTAL_BD_PER_RXQUEUE;
    }

    for (size_t queueCounter = 0; queueCounter < cfg->txQs; queueCounter++) {
        cfg->txQLen[queueCounter] = CY_ETH_DEFINE_TOTAL_BD_PER_TXQUEUE;
    }

    /** Prepare Network control Register    */
    cfg->pfcMultiQuantum   = 0;             /* bit25    pfc_ctrl              */
    cfg->enExtTsuPort      = 0;             /* bit23    ext_tsu_port_enable, Not supported by hardware  */
    cfg->storeUdpTcpOffset = 0;             /* bit22    store_udp_offset      */
    cfg->altSgmiiEn        = 0;             /* bit21    alt_sgmii_mode, Not supported by hardware       */
    cfg->enableMdio        = 0;             /* bit4     man_port_en, Bit is separately enabled during MDIO operation  */

    /** Prepare Network Configuration Register    */
    cfg->uniDirEnable    = 0;               /* bit31    uni_direction_enable, Not supported by hardware    */
    cfg->ignoreIpgRxEr   = 0;               /* bit30    ignore_ipg_rx_er, Not supported by hardware        */
    cfg->enRxBadPreamble = pstcEthIfConfig->u8enRxBadPreamble; /* bit29    nsp_change */
    // cfg->ifTypeSel    = pstcEthIfConfig->ifTypeSel;  /* bit27
    // sgmii_mode_enable  (reserved)  */
    /*          (see the following)  */
    // don't care                                                                               /* bit26 ignore_rx_fcs
    // */
    cfg->enRxHalfDupTx = 0;                 /* bit25    en_half_duplex_rx, not supported by hardware    */
    cfg->chkSumOffEn   = pstcEthIfConfig->u8chkSumOffEn;    /* bit24    receive_checksum_offload_enable */
    cfg->disCopyPause  = pstcEthIfConfig->u8disCopyPause;   /* bit23    disable_copy_of_pause_frames */

    #if defined(ETH_AXI_MASTER_PRESENT) && (ETH_AXI_MASTER_PRESENT == 1U)
    cfg->dmaBusWidth = CEDI_DMA_BUS_WIDTH_64; /* bit22:21 data bus with        */
    #else
    cfg->dmaBusWidth = CEDI_DMA_BUS_WIDTH_32; /* bit22:21 data bus with        */
    #endif

    /* 00:32bit 01:64bit    */
    cfg->mdcPclkDiv = (CEDI_MdcClkDiv)pstcEthIfConfig->mdcPclkDiv;
                                            /* bit20:18 mdc_clock_division   */
                                            /*          010: Divide 32       */
                                            /*          011: Divide 48       */

    // don't care                           /* bit17    fcs_remove           */
    cfg->rxLenErrDisc = pstcEthIfConfig->u8rxLenErrDisc;        /* bit16    length_field_error_frame_discard */
    cfg->rxBufOffset  = 0;                  /* bit15:14 receive_buffer_offset */

    // don't care                           /* bit13    pause_enable         */
    // don't care                           /* bit12    retry_test           */
    // cfg->ifTypeSel = pstcEthIfConfig->ifTypeSel /* bit11    pcs_select (reserved)*/
                                            /* (see the following)  */
    // cfg->ifTypeSel = pstcEthIfConfig->ifTypeSel /* bit10 gigabit_mode_enable  */
    /*          (see the following)  */
    cfg->extAddrMatch = 0;                  /* bit9     external_address_match_enable, not supported by hardware */
    cfg->rx1536ByteEn = pstcEthIfConfig->u8rx1536ByteEn; /* bit8     receive_1536_byte_frames */
    ...
    cfg->rxJumboFrEn = pstcEthIfConfig->u8rxJumboFrEn;  /* bit3 jumbo_frames    */
    // don't care                           /* bit2     discard_non_vlan_frames */
    cfg->fullDuplex = 1;                    /* bit1     full_duplex             */
    // cfg->ifTypeSel = pstcEthIfConfig->ifTypeSel /* bit0     speed            */

    /** configuration for cfg->ifTypeSel    */
    if ((pstcEthIfConfig->pstcWrapperConfig->stcInterfaceSel == CY_ETHIF_CTL_MII_10)   ||
        ...
        cfg->ifTypeSel = CEDI_IFSP_10M_MII;
    }
        ...
        cfg->ifTypeSel = CEDI_IFSP_100M_MII;
    }
    else {
    ...
    cfg->dmaAddrBusWidth = 0;                                   /* bit30  DMA address bus width. 0 =32b , 1=64b    */
                                                                /* 0:32bit 1:64bit         */
    cfg->enTxExtBD   = CY_ETH_DEFINE_BD;                        /* bit29  tx_bd_extended_mode_en  */
    cfg->enRxExtBD   = CY_ETH_DEFINE_BD;                        /* bit28  rx_bd_extended_mode_en  */
    cfg->dmaCfgFlags = pstcEthIfConfig->u8dmaCfgFlags;          /* bit26  force_max_amba_burst_tx */
                                                                /* bit25  force_max_amba_burst_rx */
                                                                /* bit24  force_discard_on_err    */
    for (size_t queueCounter = 0; queueCounter < cfg->rxQs; queueCounter++) {
        cfg->rxBufLength[queueCounter] =
            CY_ETH_SIZE_BUF_TXQ_RXQ >> 6; /* bit23:16 rx_buf_size       */
    }

    cfg->txPktBufSize = CY_ETH_TX_PBUF_SIZE;                    /* bit10  tx_pbuf_size            */
    cfg->rxPktBufSize = CY_ETH_RX_PBUF_SIZE;                    /* bit9:8 rx_pbuf_size            */
    cfg->dmaEndianism = 0;                                      /* bit7   endian_swap_packet  */
    ...
    cfg->dmaDataBurstLen = (CEDI_DmaDatBLen)pstcEthIfConfig->dmaDataBurstLen; /* bit4:0   amba_burst_length */
    ...
    cfg->upper32BuffTxQAddr = 0;                                /* bit31:0  not used              */
    cfg->upper32BuffRxQAddr = 0;                                /* bit31:0  not used              */

    /** axi_max_pipeline register    */
    cfg->aw2wMaxPipeline = pstcEthIfConfig->u8aw2wMaxPipeline;  /* bit15:8  aw2w_max_pipeline     */
    cfg->ar2rMaxPipeline = pstcEthIfConfig->u8ar2rMaxPipeline;  /* bit 7:0  ar2r_max_pipeline     */
}
...
static void Cy_ETHIF_AssignMemory(uint8_t u8EthIfInstance) {
    ...
    cyp_ethif_pd[u8EthIfInstance] = cy_ethif_privatedata[u8EthIfInstance];

    cfg->rxQAddr = (uintptr_t)cy_ethif_rx_desc_list[u8EthIfInstance];
    cfg->txQAddr = (uintptr_t)cy_ethif_tx_desc_list[u8EthIfInstance];
    ...
    cfg->statsRegs = (uintptr_t)cyp_ethif_statistic[u8EthIfInstance];

    /** Get the physical address */
    cfg->rxQPhyAddr = cfg->rxQAddr;

    /** Get the physical address */
    cfg->txQPhyAddr = cfg->txQAddr;
}
...
static cy_en_ethif_status_t Cy_ETHIF_DisableQueues(ETH_Type* base, cy_stc_ethif_mac_config_t* pstcEthIfConfig) {
    ...
    if (pstcEthIfConfig->btxq0enable == false) {
        ...
        ETH_TX_Q_PTR(base) = (ETH_TX_Q_PTR(base) & (uint32_t)~(ETH_TRANSMIT_Q_PTR_DMA_TX_DIS_Q_Msk)) |
                             _VAL2FLD(ETH_TRANSMIT_Q_PTR_DMA_TX_DIS_Q, 1UL);
    ...
    if (pstcEthIfConfig->btxq1enable == false) {
        /* Disable the queue pointer */
        ETH_TX_Q1_PTR(base) = (ETH_TX_Q1_PTR(base) & (uint32_t)~(ETH_TRANSMIT_Q1_PTR_DMA_TX_DIS_Q_Msk)) |
                              _VAL2FLD(ETH_TRANSMIT_Q1_PTR_DMA_TX_DIS_Q, 1UL);
    ...
    if (pstcEthIfConfig->btxq2enable == false) {
        /* Disable the queue pointer */
        ETH_TX_Q2_PTR(base) = (ETH_TX_Q2_PTR(base) & (uint32_t)~(ETH_TRANSMIT_Q2_PTR_DMA_TX_DIS_Q_Msk)) |
                              _VAL2FLD(ETH_TRANSMIT_Q2_PTR_DMA_TX_DIS_Q, 1UL);
    ...
    if (pstcEthIfConfig->brxq0enable == false) {
        /* Disable the queue pointer */
        ETH_RX_Q_PTR(base) = (ETH_RX_Q_PTR(base) & (uint32_t)~(ETH_RECEIVE_Q_PTR_DMA_RX_DIS_Q_Msk)) |
                             _VAL2FLD(ETH_RECEIVE_Q_PTR_DMA_RX_DIS_Q, 1UL);
    ...
    if (pstcEthIfConfig->brxq1enable == false) {
        /* Disable the queue pointer */
        ETH_RX_Q1_PTR(base) = (ETH_RX_Q1_PTR(base) & (uint32_t)~(ETH_RECEIVE_Q1_PTR_DMA_RX_DIS_Q_Msk)) |
                              _VAL2FLD(ETH_RECEIVE_Q1_PTR_DMA_RX_DIS_Q, 1UL);
    ...
    if (pstcEthIfConfig->brxq2enable == false) {
        /* Disable the queue pointer */
        ETH_RX_Q2_PTR(base) = (ETH_RX_Q2_PTR(base) & (uint32_t)~(ETH_RECEIVE_Q2_PTR_DMA_RX_DIS_Q_Msk)) |
                              _VAL2FLD(ETH_RECEIVE_Q2_PTR_DMA_RX_DIS_Q, 1UL);
        stcQueueDisStatus[u8EthIfInstance].bRxQueueDisable[2] = true;
    }

    /* TODO: Temporarily in the driver. MUST be removed once IP is updated || Q3 for both Transmit and Receive has been
       removed from the IP Changes will apply to bitfile after 0513 release */
...
static cy_en_ethif_status_t Cy_ETHIF_TSUInit(uint8_t u8EthIfInstance, cy_stc_ethif_tsu_config_t* pstcTSUConfig) {
    ...
    if (((uint32_t)EOK) != result) {
        /** Reason could be Null pointer, hardware does not support TSU, enTxDescStoreTimeStamp > CEDI_TX_TS_ALL,
         * enRxDescStoreTimeStamp > CEDI_RX_TS_ALL  */
        return CY_ETHIF_BAD_PARAM;
    }
...
static void Cy_ETHIF_EventTx(void* pcy_privatedata, uint32_t u32event, uint8_t u8qnum) {
    ...
    if (0UL != (u32event & ((uint32_t)CEDI_EV_TX_COMPLETE))) {
        (void) cyp_ethif_gemgxlobj->freeTxDesc(cyp_ethif_pd[u8EthIfInstance],
                                               u8qnum, &Tx_DescData);
...
static void Cy_ETHIF_EventTxError(void* pcy_privatedata, uint32_t u32event, uint8_t u8qnum) {
    ...
    (void) cyp_ethif_gemgxlobj->freeTxDesc(cyp_ethif_pd[u8EthIfInstance], u8qnum, &Tx_DescData);
...
Cy_ETHIF_DSCallbackFunc(cy_stc_syspm_callback_params_t* callbackParams,
                        cy_en_syspm_callback_mode_t mode) {
    ...
    switch (mode) {
        ...
        case CY_SYSPM_BEFORE_TRANSITION : {
            ...
            (void) emacGetSpecificAddr(pd,
                                       ...
                                       (uint8_t*)&dsreg->fileter1Config.typeFilter,
                                       (uint8_t*)&dsreg->fileter1Config.ignoreBytes);
            CY_MISRA_BLOCK_END('MISRA C-2012 Rule 11.3')

            for (size_t tmpcounter = 0; tmpcounter < cy_ethif_cfg[u8EthIfInstance].rxQs; tmpcounter++) {
                (void) emacNumRxBufs(pd, 0, dsreg->ptrRxBufs[tmpcounter]);
            }
            retVal = CY_SYSPM_SUCCESS;
            break;
        }
        ...
        case CY_SYSPM_AFTER_TRANSITION : {
            ...
            for (size_t tmpcounter = 0; tmpcounter < cy_ethif_cfg[u8EthIfInstance].rxQs; tmpcounter++) {
                if (g_pRxQbuffPool[u8EthIfInstance][tmpcounter] != NULL) {
                    for (size_t tmpintrcntr = 0;
                         tmpintrcntr < cy_ethif_cfg[u8EthIfInstance].rxQLen[tmpcounter];
                         tmpintrcntr++) {
                        rx_buff_addr.vAddr =
            ...
            (void) ctx->setSpecificAddr(pd,
                                        ...
                                        dsreg->fileter1Config.ignoreBytes);
            CY_MISRA_BLOCK_END('MISRA C-2012 Rule 11.3')
