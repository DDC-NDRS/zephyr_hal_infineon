...
#include "cy_device.h"
    ...
    (defined(CY_IP_MXS40SRSS) && (CY_IP_MXS40SRSS_VERSION >= 2)) || defined(CY_IP_MXS22SRSS)
...
static inline void Cy_SysClk_PeriPclkDecodeIpBlock(en_clk_dst_t ipBlock,
                                                   uint8_t* grpNum, uint8_t* instNum) {
...
static inline bool Cy_SysClk_PeriPclkIsDividerValid(uint8_t instNum, uint8_t grpNum,
    ...
    if (dividerType <= CY_SYSCLK_DIV_24_5_BIT) {
        switch (dividerType) {
            case CY_SYSCLK_DIV_8_BIT :
                isValid = (dividerNum < PERI_PCLK_GR_DIV_8_NR(instNum, grpNum));
                break;

            case CY_SYSCLK_DIV_16_BIT :
                isValid = (dividerNum < PERI_PCLK_GR_DIV_16_NR(instNum, grpNum));
                break;

            #if defined(CY_IP_MXSPERI) || (defined(CY_IP_MXPERI) && (CY_IP_MXPERI_VERSION >= 3))
            case CY_SYSCLK_DIV_16_5_BIT :
                isValid = (dividerNum < PERI_PCLK_GR_DIV_16_5_NR(instNum, grpNum));
                break;
            #endif

            case CY_SYSCLK_DIV_24_5_BIT :
                isValid = (dividerNum < PERI_PCLK_GR_DIV_24_5_NR(instNum, grpNum));
                break;
            default :
                break;
        }
...
static inline cy_rslt_t Cy_SysClk_PeriPclkInvokeSrf(uint8_t instNum, uint8_t grpNum, uint32_t opId,
                                                    void const* inputBase, uint32_t inputLen,
                                                    void* outputBase, uint32_t outputLen) {
    ...
    cy_pdl_invoke_srf_args invoke_args = {
        ...
        #if defined(CY_IP_MXSPERI) || (defined(CY_IP_MXPERI) && (CY_IP_MXPERI_VERSION >= 3))
        .base         = (instNum == 0U ? PERI0 : PERI1),
        .sub_block    = grpNum,
        #else
        .base         = NULL,
        .sub_block    = 0UL,
        #endif
        .input_base   = (uint8_t*)inputBase,
        .input_len    = inputLen,
        .output_base  = (uint8_t*)outputBase,
    ...
    if (outputLen != 0UL) {
        ...
        memcpy(outputBase, (&(output->output_values[0])), outputLen);
    }
...
static en_clk_dst_t Cy_SysClk_PeriPclkMakeIpBlock(uint32_t grpNum, uint32_t instNum) {
    return (en_clk_dst_t)(((grpNum  << PERI_PCLK_GR_NUM_Pos  ) & PERI_PCLK_GR_NUM_Msk) |
...
cy_rslt_t cy_pdl_sysclk_srf_peripclk_get_frac_div_impl_s(mtb_srf_input_ns_t* inputs_ns,
    ...
    Cy_SysClk_PeriPclkGetFracDivider(ipBlock, input.dividerType, input.dividerNum,
                                     &output.dividerIntValue, &output.dividerFracValue);
    status = mtb_srf_copy_output_value(outputs_ns, &output, sizeof(output));
...
cy_rslt_t cy_pdl_sysclk_srf_peripclk_set_frac_div_impl_s(mtb_srf_input_ns_t* inputs_ns,
    ...
    output.status = Cy_SysClk_PeriPclkSetFracDivider(ipBlock, input.dividerType, input.dividerNum,
                                                     input.dividerIntValue, input.dividerFracValue);
    status = mtb_srf_copy_output_value(outputs_ns, &output, sizeof(output));
...
cy_rslt_t
cy_pdl_sysclk_srf_peripclk_get_div_is_enabled_impl_s(mtb_srf_input_ns_t* inputs_ns,
                                                     mtb_srf_output_ns_t* outputs_ns,
                                                     mtb_srf_invec_ns_t* inputs_ptr_ns,
                                                     uint8_t inputs_ptr_cnt_ns,
                                                     mtb_srf_outvec_ns_t* outputs_ptr_ns,
...
cy_en_sysclk_status_t Cy_SysClk_PeriPclkSetDivider(en_clk_dst_t ipBlock, cy_en_divider_types_t dividerType,
    ...
    Cy_SysClk_PeriPclkDecodeIpBlock(ipBlock, &grpNum, &instNum);
    ...
    if (mtb_pdl_sysclk_peri_group_div_srf_secured[instNum]) {
        ...
        cy_rslt_t result = Cy_SysClk_PeriPclkInvokeSrf(instNum, grpNum, CY_PDL_SYSCLK_OP_PERIPCLK_SET_DIV,
...
uint32_t Cy_SysClk_PeriPclkGetDivider(en_clk_dst_t ipBlock, cy_en_divider_types_t dividerType, uint32_t dividerNum) {
    ...
    if (mtb_pdl_sysclk_peri_group_div_srf_secured[instNum]) {
        ...
        cy_rslt_t result = Cy_SysClk_PeriPclkInvokeSrf(instNum, grpNum, CY_PDL_SYSCLK_OP_PERIPCLK_GET_DIV,
                                                       &input_args, sizeof(input_args),
                                                       &output_args, sizeof(output_args));
        CY_ASSERT_L2(result == CY_RSLT_SUCCESS);
        CY_UNUSED_PARAMETER(result);
...
cy_en_sysclk_status_t Cy_SysClk_PeriPclkSetFracDivider(en_clk_dst_t ipBlock, cy_en_divider_types_t dividerType,
                                                       uint32_t dividerNum, uint32_t dividerIntValue,
                                                       uint32_t dividerFracValue) {
    ...
    if (mtb_pdl_sysclk_peri_group_div_srf_secured[instNum]) {
        ...
        cy_rslt_t result =
            mtb_srf_pool_allocate(&cy_pdl_srf_default_pool, &inVec, &outVec, CY_PDL_SYSCLK_SRF_POOL_TIMEOUT);
        CY_ASSERT_L2(result == CY_RSLT_SUCCESS);
    ...
    if (dividerType == CY_SYSCLK_DIV_16_5_BIT) {
        if (((uint32_t)dividerNum < (PERI_PCLK_GR_DIV_16_5_NR(instNum, grpNum))) &&
            ...
            uint32_t divRegValue = PERI_DIV_16_5_CTL(instNum, grpNum, dividerNum);

            divRegValue &= ~(PERI_DIV_16_5_CTL_INT16_DIV_Msk | PERI_DIV_16_5_CTL_FRAC5_DIV_Msk);
            divRegValue |= _VAL2FLD(PERI_DIV_16_5_CTL_INT16_DIV, dividerIntValue) |
                           _VAL2FLD(PERI_DIV_16_5_CTL_FRAC5_DIV, dividerFracValue);
            PERI_DIV_16_5_CTL(instNum, grpNum, dividerNum) = divRegValue;
            retVal = CY_SYSCLK_SUCCESS;
        }
    }
    #endif
    ...
    if (dividerType == CY_SYSCLK_DIV_24_5_BIT) {
        if (((uint32_t)dividerNum < (PERI_PCLK_GR_DIV_24_5_NR(instNum, grpNum))) &&
            ...
            uint32_t divRegValue = PERI_DIV_24_5_CTL(instNum, grpNum, dividerNum);

            divRegValue &= ~(PERI_DIV_24_5_CTL_INT24_DIV_Msk | PERI_DIV_24_5_CTL_FRAC5_DIV_Msk);
            divRegValue |= _VAL2FLD(PERI_DIV_24_5_CTL_INT24_DIV, dividerIntValue) |
                           _VAL2FLD(PERI_DIV_24_5_CTL_FRAC5_DIV, dividerFracValue);
            PERI_DIV_24_5_CTL(instNum, grpNum, dividerNum) = divRegValue;
            retVal = CY_SYSCLK_SUCCESS;
        }
    }
...
void Cy_SysClk_PeriPclkGetFracDivider(en_clk_dst_t ipBlock, cy_en_divider_types_t dividerType, uint32_t dividerNum,
    ...
    if (dividerType == CY_SYSCLK_DIV_16_5_BIT) {
        ...
        divRegValue       = PERI_DIV_16_5_CTL(instNum, grpNum, dividerNum);
        *dividerIntValue  = _FLD2VAL(PERI_DIV_16_5_CTL_INT16_DIV, divRegValue);
        *dividerFracValue = _FLD2VAL(PERI_DIV_16_5_CTL_FRAC5_DIV, divRegValue);
    }
    #endif

    if (dividerType == CY_SYSCLK_DIV_24_5_BIT) { /* 24.5-bit divider */
        CY_ASSERT_L1(dividerNum < PERI_PCLK_GR_DIV_24_5_NR(instNum, grpNum));

        uint32_t divRegValue;
        divRegValue       = PERI_DIV_24_5_CTL(instNum, grpNum, dividerNum);
        *dividerIntValue  = _FLD2VAL(PERI_DIV_24_5_CTL_INT24_DIV, divRegValue);
        *dividerFracValue = _FLD2VAL(PERI_DIV_24_5_CTL_FRAC5_DIV, divRegValue);
    }
}

cy_en_sysclk_status_t Cy_SysClk_PeriPclkAssignDivider(en_clk_dst_t ipBlock,
                                                      cy_en_divider_types_t dividerType,
                                                      uint32_t dividerNum) {
    cy_en_sysclk_status_t retVal  = CY_SYSCLK_BAD_PARAM;
    uint8_t grpNum  = 0;
    ...
    if (Cy_SysClk_PeriPclkIsDividerValid(instNum, grpNum, dividerType, dividerNum)) {
...
uint32_t Cy_SysClk_PeriPclkGetAssignedDivider(en_clk_dst_t ipBlock) {
    ...
    if (mtb_pdl_sysclk_peri_group_div_srf_secured[instNum]) {
        ...
        result = mtb_srf_pool_allocate(&cy_pdl_srf_default_pool,
                                       &inVec, &outVec, CY_PDL_SYSCLK_SRF_POOL_TIMEOUT);
        CY_ASSERT_L2(result == CY_RSLT_SUCCESS);
    ...
    return (PERI_CLOCK_CTL(instNum, grpNum, periNum) &
            (CY_PERI_CLOCK_CTL_DIV_SEL_Msk | CY_PERI_CLOCK_CTL_TYPE_SEL_Msk));
    #endif
}
...
cy_en_sysclk_status_t Cy_SysClk_PeriPclkEnableDivider(en_clk_dst_t ipBlock, cy_en_divider_types_t dividerType,
    ...
    if (mtb_pdl_sysclk_peri_group_div_srf_secured[instNum]) {
        ...
        result = Cy_SysClk_PeriPclkInvokeSrf(instNum, grpNum, CY_PDL_SYSCLK_OP_PERIPCLK_ENABLE_DIV,
                                             &input_args, sizeof(input_args),
                                             &output_args, sizeof(output_args));
        CY_ASSERT_L2(result == CY_RSLT_SUCCESS);
        CY_UNUSED_PARAMETER(result);
    ...
    if (Cy_SysClk_PeriPclkIsDividerValid(instNum, grpNum, dividerType, dividerNum)) {
        ...
        PERI_DIV_CMD(instNum, grpNum) =
            PERI_DIV_CMD_ENABLE_Msk | CY_PERI_DIV_CMD_PA_TYPE_SEL_Msk | CY_PERI_DIV_CMD_PA_DIV_SEL_Msk |
            _VAL2FLD(CY_PERI_DIV_CMD_TYPE_SEL, dividerType) | _VAL2FLD(CY_PERI_DIV_CMD_DIV_SEL, dividerNum);
...
cy_en_sysclk_status_t Cy_SysClk_PeriPclkDisableDivider(en_clk_dst_t ipBlock, cy_en_divider_types_t dividerType,
    ...
    if (mtb_pdl_sysclk_peri_group_div_srf_secured[instNum]) {
        ...
        result = Cy_SysClk_PeriPclkInvokeSrf(instNum, grpNum, CY_PDL_SYSCLK_OP_PERIPCLK_DISABLE_DIV,
                                             &input_args, sizeof(input_args),
                                             &output_args, sizeof(output_args));
        CY_ASSERT_L2(result == CY_RSLT_SUCCESS);
        CY_UNUSED_PARAMETER(result);
...
Cy_SysClk_PeriPclkEnablePhaseAlignDivider(en_clk_dst_t ipBlock,
                                          cy_en_divider_types_t dividerType,
                                          uint32_t dividerNum,
    ...
    Cy_SysClk_PeriPclkDecodeIpBlock(ipBlock, &grpNum, &instNum);
    if (Cy_SysClk_PeriPclkIsDividerValid(instNum, grpNum, dividerTypePA, dividerNumPA) ||
        ((dividerTypePA == CY_SYSCLK_DIV_24_5_BIT) && (dividerNumPA == 63u))) {
...
Cy_SysClk_PeriphSetDivider(cy_en_divider_types_t dividerType,
                           uint32_t dividerNum, uint32_t dividerValue) {
    CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.8', 'Intentional typecast to en_clk_dst_t enum.');
    return Cy_SysClk_PeriPclkSetDivider((en_clk_dst_t)PERI_PCLK_PERIPHERAL_GROUP_NUM,
                                        dividerType, dividerNum, dividerValue);
}
...
Cy_SysClk_PeriphSetFracDivider(cy_en_divider_types_t dividerType, uint32_t dividerNum,
                               uint32_t dividerIntValue, uint32_t dividerFracValue) {
    CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.8', 'Intentional typecast to en_clk_dst_t enum.');
    return Cy_SysClk_PeriPclkSetFracDivider((en_clk_dst_t)PERI_PCLK_PERIPHERAL_GROUP_NUM, dividerType,
                                            dividerNum, dividerIntValue, dividerFracValue);
}

void Cy_SysClk_PeriphGetFracDivider(cy_en_divider_types_t dividerType, uint32_t dividerNum,
                                    uint32_t* dividerIntValue, uint32_t* dividerFracValue) {
    CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.8', 'Intentional typecast to en_clk_dst_t enum.');
    Cy_SysClk_PeriPclkGetFracDivider((en_clk_dst_t)PERI_PCLK_PERIPHERAL_GROUP_NUM, dividerType, dividerNum,
                                     dividerIntValue, dividerFracValue);
}
...
Cy_SysClk_PeriphEnablePhaseAlignDivider(cy_en_divider_types_t dividerType, uint32_t dividerNum,
    ...
    CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.8', 'Intentional typecast to en_clk_dst_t enum.');
    return Cy_SysClk_PeriPclkEnablePhaseAlignDivider((en_clk_dst_t)PERI_PCLK_PERIPHERAL_GROUP_NUM, dividerType,
                                                     dividerNum, dividerTypePA, dividerNumPA);
}
...
uint32_t Cy_SysClk_ClkPumpGetFrequency(void) {
    ...
    return (Cy_SysClk_ClkPumpIsEnabled() ? (Cy_SysClk_ClkPathGetFrequency((uint32_t)Cy_SysClk_ClkPumpGetSource()) /
                                            (1UL << (uint32_t)Cy_SysClk_ClkPumpGetDivider()))
                                         : 0UL);
}
...
cy_en_sysclk_status_t
Cy_SysClk_ClkLfCsvManualConfigure(cy_en_clklf_csv_ref_clk_t refClk,
                                  cy_stc_clklf_csv_manual_config_t const* csvConfig) {
    /* Check for errors */
    ...
    CY_ASSERT_L1((int32_t)csvConfig->startTime >=
                 (((int32_t)(((float32_t)(csvConfig->period + 3UL)) * (f_refFreq / f_monitorFreq))) -
                  ((int32_t)csvConfig->upperLimit)));
...
cy_en_sysclk_status_t
Cy_SysClk_ClkLfCsvConfigure(cy_en_clklf_csv_ref_clk_t refClk, uint32_t accuracy) {
    cy_en_sysclk_status_t retVal = CY_SYSCLK_BAD_PARAM;
    ...
    if (monitorFreq > 0UL && refFreq > 0UL) {
        ...
        csvConfig.lowerLimit =
            target - (uint32_t)(((float32_t)CSV_MIN_TARGET_VAL / 100.0F) * ((float32_t)accuracy / 2.0F));
        CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.8', 'Intentional typecast to uint32_t.');
        csvConfig.upperLimit =
            target + (uint32_t)(((float32_t)CSV_MIN_TARGET_VAL / 100.0F) * ((float32_t)accuracy / 2.0F));
        CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.8', 'Intentional typecast to uint32_t.');
...
cy_en_sysclk_status_t Cy_SysClk_PeriGroupSetDivider(uint32_t groupNum, uint32_t divider) {
    ...
    if ((instNum == 0 && mtb_pdl_sysclk_peri0_group_srf_secured[groupNumMsked]) ||
        (instNum == 1 && mtb_pdl_sysclk_peri1_group_srf_secured[groupNumMsked])) {
        mtb_srf_invec_ns_t*  inVec  = NULL;
        mtb_srf_outvec_ns_t* outVec = NULL;
        mtb_srf_output_ns_t* output = NULL;
...
cy_en_sysclk_status_t
Cy_SysClk_PeriGroupSetSlaveCtl(uint32_t groupNum, cy_en_peri_grp_sl_ctl_num_t slaveCtl,
                               uint32_t value) {
    cy_en_sysclk_status_t retVal  = CY_SYSCLK_BAD_PARAM;
    uint8_t instNum = (uint8_t)(((uint32_t)groupNum & PERI_GR_INST_NUM_Msk) >> PERI_GR_INST_NUM_Pos);
...
void Cy_SysClk_ClkFastSetDivider(uint8_t divider) {
    ...
    retStatus =
        CY_PRA_FUNCTION_CALL_RETURN_PARAM(CY_PRA_MSG_TYPE_FUNC_POLICY, CY_PRA_CLK_FUNC_FAST_SET_DIVIDER, divider);
...
cy_en_clkhf_in_sources_t Cy_SysClk_ClkHfGetSource(uint32_t clkHf) {
    ...
    return (
        (cy_en_clkhf_in_sources_t)((uint32_t)(_FLD2VAL(SRSS_CLK_ROOT_SELECT_ROOT_MUX, SRSS_CLK_ROOT_SELECT[clkHf]))));
}
...
cy_en_clkhf_dividers_t Cy_SysClk_ClkHfGetDivider(uint32_t clkHf) {
    ...
    return (
        (cy_en_clkhf_dividers_t)(((uint32_t)_FLD2VAL(SRSS_CLK_ROOT_SELECT_ROOT_DIV_INT, SRSS_CLK_ROOT_SELECT[clkHf]))));
    #else
...
cy_en_sysclk_status_t
Cy_SysClk_ClkHfCsvManualConfigure(cy_stc_clkhf_csv_hf_config_t const* hfConfig,
                                  cy_stc_clkhf_csv_manual_config_t const* csvConfig) {
    /* Check for errors */
    ...
    if (hfConfig->clkHf < CY_SRSS_NUM_HFROOT) {
        ...
        CY_REG32_CLR_SET(SRSS_CSV_HF_CSV_REF_CTL(hfConfig->clkHf), CSV_HF_CSV_REF_CTL_CSV_STARTUP,
                         csvConfig->startTime - 1UL);
        CY_REG32_CLR_SET(SRSS_CSV_HF_CSV_REF_LIMIT(hfConfig->clkHf), CSV_HF_CSV_REF_LIMIT_CSV_LOWER,
                         csvConfig->lowerLimit - 1UL);
        CY_REG32_CLR_SET(SRSS_CSV_HF_CSV_REF_LIMIT(hfConfig->clkHf), CSV_HF_CSV_REF_LIMIT_CSV_UPPER,
                         csvConfig->upperLimit - 1UL);
        CY_REG32_CLR_SET(SRSS_CSV_HF_CSV_MON_CTL(hfConfig->clkHf), CSV_HF_CSV_MON_CTL_CSV_PERIOD,
                         csvConfig->period - 1UL);
        retVal = CY_SYSCLK_SUCCESS;
    }
...
cy_en_sysclk_status_t
Cy_SysClk_ClkHfCsvConfigure(cy_stc_clkhf_csv_hf_config_t const* hfConfig, uint32_t accuracy) {
    CY_ASSERT_L1(hfConfig != NULL);
    CY_ASSERT_L1(hfConfig->clkHf < CY_SRSS_NUM_HFROOT);
    ...
    if (hfConfig->clkHf < CY_SRSS_NUM_HFROOT) {
        ...
        CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.8', 'Intentional typecast to uint32_t.');
        csvConfig.lowerLimit =
            (uint32_t)((float32_t)target - (((float32_t)CSV_MIN_TARGET_VAL / 100.0F) * ((float32_t)accuracy / 2.0F)));
...
void Cy_SysClk_MfoEnable(bool deepSleepEnable) {
    ...
    #else
    SRSS_CLK_MFO_CONFIG =
        SRSS_CLK_MFO_CONFIG_ENABLE_Msk | (deepSleepEnable ? SRSS_CLK_MFO_CONFIG_DPSLP_ENABLE_Msk : 0UL);
    #endif
    #endif
...
uint32_t Cy_SysClk_EcoBleGetStatus(void) {
    ...
    return ((SRSS_CLK_ECO_STATUS_ECO_BLE_ENABLED_Msk == (SRSS_CLK_ECO_STATUS_ECO_BLE_ENABLED_Msk & SRSS_CLK_ECO_STATUS))
                ? CY_SYSCLK_ECOSTAT_BLE_ENABLED
                : CY_SYSCLK_ECOSTAT_BLE_DISABLED);
    #else
    return 0;
    #endif
    #else
...
/* ========================================================================== */
#if (defined(CY_IP_MXS40SRSS) && (CY_IP_MXS40SRSS_VERSION >= 3)) && \
    (defined(SRSS_BACKUP_S40E_LPECO_PRESENT) && (SRSS_BACKUP_S40E_LPECO_PRESENT == 1u))
...
void Cy_SysClk_LpEcoConfigure(cy_en_clkbak_lpeco_loadcap_range_t   capValue,
                              ...
                              cy_en_clkbak_lpeco_max_amplitude_t ampValue, bool ampDetEn) {
...
#endif /* (defined (CY_IP_MXS40SRSS) && (CY_IP_MXS40SRSS_VERSION >= 3)) && (defined (SRSS_BACKUP_S40E_LPECO_PRESENT)   \
          && (SRSS_BACKUP_S40E_LPECO_PRESENT == 1u)) */
...
cy_en_sysclk_status_t Cy_SysClk_ClkPathSetSource(uint32_t clkPath, cy_en_clkpath_in_sources_t source) {
    ...
    if (clkPath < CY_SRSS_NUM_CLKPATH) {
        if (source >= CY_SYSCLK_CLKPATH_IN_DSI) {
            ...
            SRSS_CLK_PATH_SELECT[clkPath] =
                _VAL2FLD(SRSS_CLK_PATH_SELECT_PATH_MUX, (uint32_t)CY_SYSCLK_CLKPATH_IN_DSIMUX);
        }
        else {
...
cy_en_clkpath_in_sources_t Cy_SysClk_ClkPathGetSource(uint32_t clkPath) {
    ...
    if (retVal == CY_SYSCLK_CLKPATH_IN_DSIMUX) {
        retVal = (cy_en_clkpath_in_sources_t)((
            uint32_t)(((uint32_t)CY_SYSCLK_CLKPATH_IN_DSI) |
...
uint32_t Cy_SysClk_ClkPathGetFrequency(uint32_t clkPath) {
    ...
    else if ((clkPath > 0UL) && (clkPath <= CY_SRSS_NUM_PLL) &&
             Cy_SysClk_PllIsEnabled(clkPath)) { /* PLL? (always path 1...N)*/
        freq = Cy_SysClk_PllGetFrequency(clkPath);
    }
    #endif /* CY_SRSS_PLL_PRESENT */
    else {
...
cy_en_sysclk_status_t Cy_SysClk_FllDisable(void) {
    ...
    for (; (((uint32_t)CY_SYSCLK_FLLPLL_OUTPUT_INPUT) !=
            _FLD2VAL(SRSS_CLK_FLL_CONFIG3_BYPASS_SEL, SRSS_CLK_FLL_CONFIG3)) &&
           (0UL != timeoutus);
         timeoutus--) {
        Cy_SysLib_DelayUs(1U);
    }
...
cy_en_sysclk_status_t Cy_SysClk_FllConfigure(uint32_t inputFreq, uint32_t outputFreq,
                                             cy_en_fll_pll_output_mode_t outputMode) {
    #if ((defined CY_SRSS_FLL_PRESENT) && (CY_SRSS_FLL_PRESENT == 1U))
    cy_en_sysclk_status_t retVal = CY_SYSCLK_SUCCESS;

    /* check for errors */
    if ((outputFreq < CY_SYSCLK_FLL_MIN_OUTPUT_FREQ) ||
        (CY_SYSCLK_FLL_MAX_OUTPUT_FREQ < outputFreq) || /* invalid output frequency */
        ...
        if (outputMode != CY_SYSCLK_FLLPLL_OUTPUT_INPUT) {
                ...
                ((ccoFreq >= 150339200UL)
                     ...
                     : ((ccoFreq >= 113009380UL)
                            ...
                            : ((ccoFreq >= 84948700UL)
                                   ...
                                   : ((ccoFreq >= 63855600UL) ? CY_SYSCLK_FLL_CCO_RANGE1
            ...
            config.refDiv =
                wcoSource ? 19U : (uint16_t)CY_SYSLIB_DIV_ROUNDUP((uint64_t)inputFreq * 250ULL, (uint64_t)outputFreq);

            /* 5. Compute the FLL multiplier value.
                  Formula is fllMult = ccoFreq / (inputFreq / refDiv) */
            config.fllMult =
                (uint32_t)CY_SYSLIB_DIV_ROUNDUP((uint64_t)ccoFreq * (uint64_t)config.refDiv, (uint64_t)inputFreq);
...

            {
                ...
                uint32_t const trimSteps[] = {110340UL, 110200UL, 110000UL, 110000UL, 117062UL}; /* Scaled by 10^8 */
                uint32_t const margin[]    = {436UL, 581UL, 772UL, 1030UL, 1320UL};              /* Scaled by 10^5 */

                /* 7. Compute the CCO igain and pgain */
                {
                    /* intermediate parameters */
                    uint32_t kcco = (trimSteps[config.ccoRange] * margin[config.ccoRange]);
                    uint32_t ki_p = (uint32_t)CY_SYSLIB_DIV_ROUND(850ULL * CY_SYSCLK_FLL_INT_COEF * inputFreq,
                                                                  (uint64_t)kcco * (uint64_t)config.refDiv);
                ...
                {
                    uint64_t cmp = CY_SYSLIB_DIV_ROUND(((TRIM_STEPS_SCALE / MARGIN_SCALE) * (uint64_t)ccoFreq),
                                                       (uint64_t)margin[config.ccoRange]);
            ...
            /* 9. Compute the settling count, using a 1 usec settling time. Use a constant if the WCO is the FLL source */
            {
                ...
                config.settlingCount =
                    wcoSource ? 200U : (uint16_t)((outputFreq < fref) ? divval : ((divval > altval) ? divval : altval));
            }
...
cy_en_sysclk_status_t Cy_SysClk_FllManualConfigure(cy_stc_fll_manual_config_t const* config) {
    #if ((defined CY_SRSS_FLL_PRESENT) && (CY_SRSS_FLL_PRESENT == 1U))
    cy_en_sysclk_status_t retVal = CY_SYSCLK_INVALID_STATE;
    ...
    if (!Cy_SysClk_FllIsEnabled()) { /* If disabled */
        ...
        CY_ASSERT_L1(config->settlingCount <=
                     (SRSS_CLK_FLL_CONFIG3_SETTLING_COUNT_Msk >> SRSS_CLK_FLL_CONFIG3_SETTLING_COUNT_Pos));
...
uint32_t Cy_SysClk_FllGetFrequency(void) {
    ...
    cy_stc_fll_manual_config_t fllCfg = {0UL, 0U, CY_SYSCLK_FLL_CCO_RANGE0,     false, 0U, 0U,
                                         0U,  0U, CY_SYSCLK_FLLPLL_OUTPUT_AUTO, 0U};
    ...
    /* If FLL is enabled and not bypassed and avoid division by zero */
    if (enabled && (0UL != rDiv)) {
        freq = Cy_SysClk_ClkPathMuxGetFrequency(0UL); /* FLL mapped always to path 0 */
        freq = (uint32_t)CY_SYSLIB_DIV_ROUND(((uint64_t)freq * (uint64_t)fDiv), ((uint64_t)rDiv * (uint64_t)oDiv));
    }
...
#endif /* defined (CY_IP_MXS40SRSS) || defined (CY_IP_MXS40SSRSS) || (defined (CY_IP_MXS40SRSS) && \
          (CY_IP_MXS40SRSS_VERSION < 2)) */
/* ========================================================================== */
/* ===========================    PLL SECTION    ============================ */
/* ========================================================================== */
...
#if (defined(CY_IP_MXS40SRSS) && (CY_IP_MXS40SRSS_VERSION >= 2)) || defined(CY_IP_MXS22SRSS) || \
    (defined(CY_IP_MXS40SSRSS) && (SRSS_NUM_TOTAL_PLL > 0UL))
...
cy_en_sysclk_status_t Cy_SysClk_Pll400MConfigure(uint32_t pllNum, cy_stc_pll_config_t const* config) {
    CY_UNUSED_PARAMETER(pllNum);
    ...
    if (((config->inputFreq) < CY_SYSCLK_PLL400M_MIN_IN_FREQ) ||
        (CY_SYSCLK_PLL400M_MAX_IN_FREQ < (config->inputFreq)) ||
        ((config->outputFreq) < CY_SYSCLK_PLL400M_MIN_OUT_FREQ) ||
        (CY_SYSCLK_PLL400M_MAX_OUT_FREQ < (config->outputFreq))) {
        retVal = CY_SYSCLK_BAD_PARAM;
    }
    else {
        cy_stc_pll_manual_config_t manualConfig = {
            0U,    0U, 0U, false, CY_SYSCLK_FLLPLL_OUTPUT_AUTO, 0, false,
            false, 0,  0,  false
        };
                ...
                for (p = CY_SYSCLK_PLL400M_MIN_FB_DIV; p <= CY_SYSCLK_PLL400M_MAX_FB_DIV; p++) {
                    ...
                    if ((CY_SYSCLK_PLL400M_MIN_FVCO <= fvco) && (fvco <= CY_SYSCLK_PLL400M_MAX_FVCO)) {
                        ...
                        for (out = CY_SYSCLK_PLL400M_MIN_OUTPUT_DIV; out <= CY_SYSCLK_PLL400M_MAX_OUTPUT_DIV; out++) {
                            ...
                            uint64_t tempFeedBackDivLeftShifted =
                                ((tempVco << (uint64_t)SRSS_PLL400M_FRAC_BIT_COUNT) * (uint64_t)q) /
                                (uint64_t)config->inputFreq;
                            volatile uint32_t feedBackFracDiv =
                                (uint32_t)(tempFeedBackDivLeftShifted &
                                           ((1ULL << (uint64_t)SRSS_PLL400M_FRAC_BIT_COUNT) - 1ULL));
                            ...
                            uint32_t fout = (uint32_t)((((uint64_t)config->inputFreq *
                                                         (((uint64_t)p << SRSS_PLL400M_FRAC_BIT_COUNT) +
                                                          (uint64_t)feedBackFracDiv)) /
                                                        ((uint64_t)q * (uint64_t)out)) >>
                                                       SRSS_PLL400M_FRAC_BIT_COUNT);
...
cy_en_sysclk_status_t Cy_SysClk_Pll400MManualConfigure(uint32_t pllNum, cy_stc_pll_manual_config_t const* config) {
    CY_UNUSED_PARAMETER(pllNum);
    CY_UNUSED_PARAMETER(config);
    ...
    else if ((config->outputDiv < CY_SYSCLK_PLL400M_MIN_OUTPUT_DIV) ||
             (CY_SYSCLK_PLL400M_MAX_OUTPUT_DIV < config->outputDiv) ||
             (config->referenceDiv < CY_SYSCLK_PLL400M_MIN_REF_DIV) ||
             (CY_SYSCLK_PLL400M_MAX_REF_DIV < config->referenceDiv) ||
             (config->feedbackDiv < CY_SYSCLK_PLL400M_MIN_FB_DIV) ||
             (CY_SYSCLK_PLL400M_MAX_FB_DIV < config->feedbackDiv)) {
        retVal = CY_SYSCLK_BAD_PARAM;
    }
    else { /* no errors */
        /* If output mode is bypass (input routed directly to output), then done.
...
cy_en_sysclk_status_t Cy_SysClk_Pll400MEnable(uint32_t pllNum, uint32_t timeoutus) {
    ...
    if (zeroTimeout || (0UL != timeoutus)) {
        ...
        if ((uint32_t)CY_SYSCLK_FLLPLL_OUTPUT_INPUT ==
            (uint32_t)_FLD2VAL(CLK_PLL400M_CONFIG_BYPASS_SEL, SRSS_CLK_PLL_400M_CONFIG(pllNum))) {
            CY_REG32_CLR_SET(SRSS_CLK_PLL_400M_CONFIG(pllNum), CLK_PLL400M_CONFIG_BYPASS_SEL,
...
uint32_t Cy_SysClk_Pll400MGetFrequency(uint32_t pllNum) {
    ...
    if (enabled &&                          /* If PLL is enabled and not bypassed */
        ...
        freq = (uint32_t)((((uint64_t)freq * (((uint64_t)fDiv << SRSS_PLL400M_FRAC_BIT_COUNT) + (uint64_t)fracDiv)) /
...
cy_en_sysclk_status_t Cy_SysClk_Pll200MConfigure(uint32_t pllNum, cy_stc_pll_config_t const* config) {
    cy_en_sysclk_status_t retVal = CY_SYSCLK_SUCCESS;
...
cy_en_sysclk_status_t Cy_SysClk_Pll200MManualConfigure(uint32_t pllNum, cy_stc_pll_manual_config_t const* config) {
    cy_en_sysclk_status_t retVal = CY_SYSCLK_SUCCESS;
...
cy_en_sysclk_status_t Cy_SysClk_Pll200MEnable(uint32_t pllNum, uint32_t timeoutus) {
    ...
    if (zeroTimeout || (0UL != timeoutus)) {
        /* Unbypass PLL, if it is not in AUTO mode */
        if ((uint32_t)CY_SYSCLK_FLLPLL_OUTPUT_INPUT ==
            (uint32_t)_FLD2VAL(SRSS_CLK_PLL_CONFIG_BYPASS_SEL, SRSS_CLK_PLL_CONFIG[pllNum])) {
            CY_REG32_CLR_SET(SRSS_CLK_PLL_CONFIG[pllNum], SRSS_CLK_PLL_CONFIG_BYPASS_SEL,
                             CY_SYSCLK_FLLPLL_OUTPUT_OUTPUT);
        }
...
cy_en_sysclk_status_t Cy_SysClk_DpllLpConfigure(uint32_t pllNum, cy_stc_pll_config_t const* config) {
    CY_UNUSED_PARAMETER(pllNum);
    CY_UNUSED_PARAMETER(config);
    ...
    if (((config->inputFreq) < CY_SYSCLK_DPLL_LP_MIN_IN_FREQ) ||
        (CY_SYSCLK_DPLL_LP_MAX_IN_FREQ < (config->inputFreq)) ||
        ((config->outputFreq) < CY_SYSCLK_DPLL_LP_MIN_OUT_FREQ) ||
        (CY_SYSCLK_DPLL_LP_MAX_OUT_FREQ < (config->outputFreq))) {
        retVal = CY_SYSCLK_BAD_PARAM;
    }
    else {
        ...
        if (config->outputMode != CY_SYSCLK_FLLPLL_OUTPUT_INPUT) {
            ...
            for (q = CY_SYSCLK_DPLL_LP_MIN_REF_DIV; q <= CY_SYSCLK_DPLL_LP_MAX_REF_DIV; q++) {
                ...
                for (p = CY_SYSCLK_DPLL_LP_MIN_FB_DIV; p <= CY_SYSCLK_DPLL_LP_MAX_FB_DIV; p++) {
                    ...
                    if (((CY_SYSCLK_DPLL_LP_MIN_FDCO <= fdco) && (fdco <= CY_SYSCLK_DPLL_LP_MAX_FDCO)) &&
                        ((CY_SYSCLK_DPLL_LP_MIN_PDF <= pdf) && (pdf <= CY_SYSCLK_DPLL_LP_MAX_PDF))) {
                        /* OUTPUT_DIV selection */
                        for (out = CY_SYSCLK_DPLL_LP_MIN_OUTPUT_DIV; out <= CY_SYSCLK_DPLL_LP_MAX_OUTPUT_DIV; out++) {
                            ...
                            uint64_t tempFeedBackDivLeftShifted =
                                ((tempVco << (uint64_t)SRSS_DPLL_LP_FRAC_BIT_COUNT) * (uint64_t)q) /
                                (uint64_t)config->inputFreq;
                            volatile uint32_t feedBackFracDiv =
                                (uint32_t)(tempFeedBackDivLeftShifted &
                                           ((1ULL << (uint64_t)SRSS_DPLL_LP_FRAC_BIT_COUNT) - 1ULL));
                            /* Calculate what output frequency will actually be produced.
                               If it's closer to the target than what we have so far, then save it. */
                            uint32_t fout = (uint32_t)((((uint64_t)config->inputFreq *
                                                         (((uint64_t)p << SRSS_DPLL_LP_FRAC_BIT_COUNT) +
                                                          (uint64_t)feedBackFracDiv)) /
                                                        ((uint64_t)q * (uint64_t)out)) >>
                                                       SRSS_DPLL_LP_FRAC_BIT_COUNT);
...
cy_en_sysclk_status_t Cy_SysClk_DpllLpManualConfigure(uint32_t pllNum, cy_stc_pll_manual_config_t const* config) {
    CY_UNUSED_PARAMETER(pllNum);
    CY_UNUSED_PARAMETER(config);
...
cy_en_sysclk_status_t Cy_SysClk_DpllLpEnable(uint32_t pllNum, uint32_t timeoutus) {
    ...
    if (zeroTimeout || (0UL != timeoutus)) {
        /* Unbypass PLL, if it is not in AUTO mode */
        if ((uint32_t)CY_SYSCLK_FLLPLL_OUTPUT_INPUT ==
            (uint32_t)_FLD2VAL(CLK_DPLL_LP_CONFIG_BYPASS_SEL, SRSS_CLK_DPLL_LP_CONFIG(pllNum))) {
            CY_REG32_CLR_SET(SRSS_CLK_DPLL_LP_CONFIG(pllNum), CLK_DPLL_LP_CONFIG_BYPASS_SEL,
                             CY_SYSCLK_FLLPLL_OUTPUT_OUTPUT);
        }
...
uint32_t Cy_SysClk_DpllLpGetFrequency(uint32_t pllNum) {
    ...
    if (enabled &&                          /* If PLL is enabled and not bypassed */
        ...
        freq = (uint32_t)((((uint64_t)freq * (((uint64_t)fDiv << SRSS_DPLL_LP_FRAC_BIT_COUNT) + (uint64_t)fracDiv)) /
                           ((uint64_t)rDiv * (uint64_t)oDiv)) >>
                          SRSS_DPLL_LP_FRAC_BIT_COUNT);
    }
...
cy_en_sysclk_status_t Cy_SysClk_DpllHpConfigure(uint32_t pllNum, cy_stc_pll_config_t const* config) {
    CY_UNUSED_PARAMETER(pllNum);
    CY_UNUSED_PARAMETER(config);
    ...
    if (((config->inputFreq) < CY_SYSCLK_DPLL_HP_MIN_IN_FREQ)   ||
        (CY_SYSCLK_DPLL_HP_MAX_IN_FREQ < (config->inputFreq))   ||
        ((config->outputFreq) < CY_SYSCLK_DPLL_HP_MIN_OUT_FREQ) ||
        (CY_SYSCLK_DPLL_HP_MAX_OUT_FREQ < (config->outputFreq))) {
        retVal = CY_SYSCLK_BAD_PARAM;
    }
    else {
        ...
        if (config->outputMode != CY_SYSCLK_FLLPLL_OUTPUT_INPUT) {
            ...
            for (pDiv = CY_SYSCLK_DPLL_HP_MIN_PDIV; pDiv <= CY_SYSCLK_DPLL_HP_MAX_PDIV; pDiv++) {
                ...
                for (nDiv = CY_SYSCLK_DPLL_HP_MIN_NDIV; nDiv <= CY_SYSCLK_DPLL_HP_MAX_NDIV; nDiv++) {
                    ...
                    if (((CY_SYSCLK_DPLL_HP_MIN_FDCO <= fdco) && (fdco <= CY_SYSCLK_DPLL_HP_MAX_FDCO)) &&
                        ((CY_SYSCLK_DPLL_HP_MIN_PDF <= pdf) && (pdf <= CY_SYSCLK_DPLL_HP_MAX_PDF))) {
                        /* KDIV (kDiv) selection */
                        for (kDiv = CY_SYSCLK_DPLL_HP_MIN_KDIV; kDiv <= CY_SYSCLK_DPLL_HP_MAX_KDIV; kDiv++) {
                            ...
                            uint64_t tempFeedBackDivLeftShifted =
                                ((tempVco << (uint64_t)SRSS_DPLL_HP_FRAC_BIT_COUNT) * (uint64_t)pDiv) /
                                (uint64_t)config->inputFreq;
                            volatile uint32_t feedBackFracDiv =
                                (uint32_t)(tempFeedBackDivLeftShifted &
                                           ((1ULL << (uint64_t)SRSS_DPLL_HP_FRAC_BIT_COUNT) - 1ULL));
                            /* Calculate what output frequency will actually be produced.
                               If it's closer to the target than what we have so far, then save it. */
                            uint32_t fout = (uint32_t)((((uint64_t)config->inputFreq *
                                                         (((uint64_t)nDiv << SRSS_DPLL_HP_FRAC_BIT_COUNT) +
                                                          (uint64_t)feedBackFracDiv)) /
                                                        ((uint64_t)pDiv * (uint64_t)kDiv)) >>
                                                       SRSS_DPLL_HP_FRAC_BIT_COUNT);
        ...
        manualConfig.hpPllCfg->betaExt =
            (uint8_t)Cy_SysClk_DpllHpGetLfBeta(config->inputFreq, manualConfig.hpPllCfg->pDiv);
...
cy_en_sysclk_status_t
Cy_SysClk_DpllHpManualConfigure(uint32_t pllNum, cy_stc_pll_manual_config_t const* config) {
    CY_UNUSED_PARAMETER(pllNum);
    CY_UNUSED_PARAMETER(config);
...
cy_en_sysclk_status_t Cy_SysClk_DpllHpEnable(uint32_t pllNum, uint32_t timeoutus) {
    ...
    if (zeroTimeout || (0UL != timeoutus)) {
        /* Unbypass PLL, if it is not in AUTO mode */
        if ((uint32_t)CY_SYSCLK_FLLPLL_OUTPUT_INPUT ==
            (uint32_t)_FLD2VAL(CLK_DPLL_HP_CONFIG_BYPASS_SEL, SRSS_CLK_DPLL_HP_CONFIG(pllNum))) {
            CY_REG32_CLR_SET(SRSS_CLK_DPLL_HP_CONFIG(pllNum), CLK_DPLL_HP_CONFIG_BYPASS_SEL,
                             CY_SYSCLK_FLLPLL_OUTPUT_OUTPUT);
        }
...
uint32_t Cy_SysClk_DpllHpGetFrequency(uint32_t pllNum) {
    ...
    if (enabled) { /* If PLL is enabled and not bypassed */
        ...
        if (0U == nDivFract) {
            CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.3',
            ...
            CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.3',
                                  'unsigned 64-bit int" to different or narrower essential type "unsigned 32-bit int');
            freq =
                (uint32_t)((((uint64_t)freq * (((uint64_t)nDiv << SRSS_DPLL_HP_FRAC_BIT_COUNT) + (uint64_t)nDivFract)) /
                            ((uint64_t)pDiv * (uint64_t)kDiv)) >>
                           SRSS_DPLL_HP_FRAC_BIT_COUNT);
        }
    }
...
cy_en_sysclk_status_t Cy_SysClk_PllConfigure(uint32_t clkPath, cy_stc_pll_config_t const* config) {
    #if defined(CY_IP_MXS22SRSS)
    CY_ASSERT_L1(clkPath < (CY_SRSS_NUM_PLL));
...
cy_en_sysclk_status_t Cy_SysClk_PllManualConfigure(uint32_t clkPath, cy_stc_pll_manual_config_t const* config) {
    #if defined(CY_IP_MXS22SRSS)
    CY_ASSERT_L1(clkPath < (CY_SRSS_NUM_PLL));
...
cy_en_sysclk_status_t Cy_SysClk_StartClkMeasurementCounters(cy_en_meas_clks_t clock1, uint32_t count1,
    ...
    */
    if ((clock1 < CY_SYSCLK_MEAS_CLK_LAST_CLK) && (clock2 < CY_SYSCLK_MEAS_CLK_LAST_CLK) &&
        ...
        if (clock1 < CY_SYSCLK_MEAS_CLK_FAST_CLKS) { /* slow clock */
            ...
            if (clock1 < CY_SYSCLK_MEAS_CLK_PATH_CLKS) { /* ECO, EXT, ALTHF */
                clkOutputFastVal |= _VAL2FLD(SRSS_CLK_OUTPUT_FAST_FAST_SEL0, (uint32_t)clock1);
                clkOutputFastMask |= SRSS_CLK_OUTPUT_FAST_FAST_SEL0_Msk;
            }
            else { /* PATH or CLKHF */
                clkOutputFastVal |= _VAL2FLD(SRSS_CLK_OUTPUT_FAST_FAST_SEL0,
                                             (((uint32_t)clock1 >> 8) & 0xFUL) /*use enum bits [11:8]*/);
                clkOutputFastMask |= SRSS_CLK_OUTPUT_FAST_FAST_SEL0_Msk;

                if (clock1 < CY_SYSCLK_MEAS_CLK_CLKHFS) { /* PATH select */
                    clkOutputFastVal |=
                        _VAL2FLD(SRSS_CLK_OUTPUT_FAST_PATH_SEL0, ((uint32_t)clock1 & 0xFUL) /*use enum bits [3:0]*/);
                    clkOutputFastMask |= SRSS_CLK_OUTPUT_FAST_PATH_SEL0_Msk;
                }
                else { /* CLKHF select */
                    clkOutputFastVal |=
                        _VAL2FLD(SRSS_CLK_OUTPUT_FAST_HFCLK_SEL0, ((uint32_t)clock1 & 0xFUL) /*use enum bits [3:0]*/);
                    clkOutputFastMask |= SRSS_CLK_OUTPUT_FAST_HFCLK_SEL0_Msk;
                }
            }
        } /* clock1 fast clock */

        if (clock2 < CY_SYSCLK_MEAS_CLK_FAST_CLKS) { /* slow clock */
            ...
            if (clock2 < CY_SYSCLK_MEAS_CLK_PATH_CLKS) { /* ECO, EXT, ALTHF */
                clkOutputFastVal |= _VAL2FLD(SRSS_CLK_OUTPUT_FAST_FAST_SEL1, (uint32_t)clock2);
                clkOutputFastMask |= SRSS_CLK_OUTPUT_FAST_FAST_SEL1_Msk;
            }
            else { /* PATH or CLKHF */
                clkOutputFastVal |= _VAL2FLD(SRSS_CLK_OUTPUT_FAST_FAST_SEL1,
                                             (((uint32_t)clock2 >> 8) & 0xFUL) /*use enum bits [11:8]*/);
                clkOutputFastMask |= SRSS_CLK_OUTPUT_FAST_FAST_SEL1_Msk;

                if (clock2 < CY_SYSCLK_MEAS_CLK_CLKHFS) { /* PATH select */
                    clkOutputFastVal |=
                        _VAL2FLD(SRSS_CLK_OUTPUT_FAST_PATH_SEL1, ((uint32_t)clock2 & 0xFUL) /*use enum bits [3:0]*/);
                    clkOutputFastMask |= SRSS_CLK_OUTPUT_FAST_PATH_SEL1_Msk;
                }
                else { /* CLKHF select */
                    clkOutputFastVal |=
                        _VAL2FLD(SRSS_CLK_OUTPUT_FAST_HFCLK_SEL1, ((uint32_t)clock2 & 0xFUL) /*use enum bits [3:0]*/);
...
/** \endcond */
CY_MISRA_DEVIATE_BLOCK_START('MISRA C-2012 Rule 18.1', 5, 'Suppressing taking true and false branch.')
...
uint32_t Cy_Sysclk_PeriPclkGetClkHfNum(uint32_t ipBlock) {
    ...
    static const uint32_t peri0GrpToHfArray[] = {
        PERI0_PCLK_GR_NUM_0_CLK_HF_NUM,
    ...
    #elif defined(CY_IP_MXS22SRSS)
    static const uint32_t peri0GrpToHfArray[PERI0_PCLK_GROUP_NR] = {
        PERI0_PCLK_GR_NUM_0_CLK_HF_NUM,
        PERI0_PCLK_GR_NUM_1_CLK_HF_NUM,
        PERI0_PCLK_GR_NUM_2_CLK_HF_NUM,
        PERI0_PCLK_GR_NUM_3_CLK_HF_NUM,
    ...
    #if defined(CY_IP_MXS22SRSS)
    static const uint32_t peri1GrpToHfArray[PERI1_PCLK_GROUP_NR] = {
        PERI1_PCLK_GR_NUM_0_CLK_HF_NUM,
...
uint32_t Cy_SysClk_PeriPclkGetFrequency(en_clk_dst_t ipBlock, cy_en_divider_types_t dividerType, uint32_t dividerNum) {
    ...
    if (mtb_pdl_sysclk_peri_group_div_srf_secured[instNum]) {
        ...
        result = Cy_SysClk_PeriPclkInvokeSrf((uint8_t)instNum, (uint8_t)grpNum,
                                             CY_PDL_SYSCLK_OP_PERIPCLK_GET_FREQ, &input_args,
                                             sizeof(input_args), &output_args, sizeof(output_args));
        CY_ASSERT_L2(result == CY_RSLT_SUCCESS);
        CY_UNUSED_PARAMETER(result);
    ...
    #else
    CY_ASSERT_L1(
        ((dividerType == CY_SYSCLK_DIV_8_BIT)    && (dividerNum < PERI_PCLK_GR_DIV_8_NR(instNum, grpNum)))    ||
        ((dividerType == CY_SYSCLK_DIV_16_BIT)   && (dividerNum < PERI_PCLK_GR_DIV_16_NR(instNum, grpNum)))   ||
        ((dividerType == CY_SYSCLK_DIV_16_5_BIT) && (dividerNum < PERI_PCLK_GR_DIV_16_5_NR(instNum, grpNum))) ||
        ((dividerType == CY_SYSCLK_DIV_24_5_BIT) && (dividerNum < PERI_PCLK_GR_DIV_24_5_NR(instNum, grpNum))));
    #endif
...
cy_en_sysclk_status_t
Cy_SysClk_ClkRefCsvManualConfigure(cy_en_clkref_csv_ref_clk_t refClk,
                                   cy_stc_clkref_csv_manual_config_t const* csvConfig) {
    /* Check for errors */
    CY_ASSERT_L1(csvConfig != NULL);
...
cy_en_sysclk_status_t
Cy_SysClk_ClkRefCsvConfigure(cy_en_clkref_csv_ref_clk_t refClk, uint32_t accuracy) {
    cy_en_sysclk_status_t retVal = CY_SYSCLK_BAD_PARAM;
...
cy_en_sysclk_status_t
Cy_SysClk_ClkIloCsvManualConfigure(cy_en_clkilo_csv_ref_clk_t refClk,
                                   cy_stc_clkilo_csv_manual_config_t const* csvConfig) {
    /* Check for errors */
    CY_ASSERT_L1(csvConfig != NULL);
