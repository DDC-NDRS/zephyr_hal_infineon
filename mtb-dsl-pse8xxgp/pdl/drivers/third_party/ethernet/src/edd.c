...
static void calcDescriptorSizes(CEDI_Config const* config,
                                uint16_t* txDescSize,
                                uint16_t* rxDescSize) {
...
static uint32_t numTxDescriptors(CEDI_Config* config) {
    uint32_t sumTxDesc = 0;

    for (size_t i = 0; i < config->txQs; i++) {
...
static uint32_t numRxDescriptors(CEDI_Config* config) {
    uint32_t sumRxDesc = 0;

    for (size_t i = 0; i < config->rxQs; i++) {
        /* allow 1 extra for "endstop" descriptor */
...
static uint32_t initTxDescLists(void* pD) {
     ...
     * privateData struct */
    CEDI_PdVar(txQueue[0]).vAddrList = (uintptr_t*)((uint8_t*)pD + sizeof(CEDI_PrivateData));
    for (size_t q = 0; q < CEDI_PdVar(cfg).txQs; q++) {
...
uint32_t initRxDescLists(void* pD) {
    ...
    CEDI_PdVar(rxQueue[0]).rxBufVAddr = (uintptr_t*)((uint8_t*)pD + sizeof(CEDI_PrivateData) +
                                                     sizeof(uintptr_t) * (numTxDescriptors(&CEDI_PdVar(cfg))));

    for (size_t q = 0; q < CEDI_PdVar(cfg).rxQs; q++) {
        ...
        for (size_t i = 0; i < rxQ->numRxDesc; i++) {
...
static uint8_t maxHwQs(uintptr_t regBase) {
    size_t qCount = 1;
    uint32_t reg = CPS_UncachedRead32((&(((struct emac_regs*)(regBase))->designcfg_debug6)));

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE1__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE2__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE3__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE4__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE5__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE6__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE7__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE8__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE9__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE10__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE11__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE12__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE13__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE14__READ(reg)) {
        qCount++;
    }

    if (EMAC_REGS__DESIGNCFG_DEBUG6__DMA_PRIORITY_QUEUE15__READ(reg)) {
        qCount++;
    }
...
static void clearAllInterrupts(void* pD) {
    ...
    #define CEDI_CLEAR_ALL_WRCLR_INT(Q) \
    if (CEDI_PdVar(numQs) > Q)          \
        CPS_UncachedWrite32(CEDI_RegAddr(int_q##Q##_status), 0xFFFFFFFF);

    #define CEDI_CLEAR_ALL_RDCLR_INT(Q) \
    if (CEDI_PdVar(numQs) > Q)          \
...
static void initAxiMaxPipelineReg(void* pD) {
    ...
    axiPipelineFifoDepth = 1UL << (CEDI_PdVar(hwCfg).axi_access_pipeline_bits);
...
static void initNetConfigReg(void* pD) {
    ...
    if (config->rxLenErrDisc) {
        EMAC_REGS__NETWORK_CONFIG__LENGTH_FIELD_ERROR_FRAME_DISCARD__SET(regTmp);
    }
...
static void setRxQBufferSizes(void* pD, CEDI_Config const* config) {
    #define CEDI_WR_RXQB_SIZE_N_CASE(Q) \
    case Q :                            \
        CPS_UncachedWrite32(CEDI_RegAddr(dma_rxbuf_size_q##Q), reg); \
        break;

    uint32_t reg;
    if (CEDI_PdVar(cfg).rxQs > 1) {
        for (size_t q = 1; q <= CEDI_PdVar(cfg).rxQs; q++) {
            reg = 0;
...
uint32_t readDesignConfig(void* pD) {
    ...
    CEDI_DesignCfg* hwCfg = &CEDI_PdVar(hwCfg);

    /* read in revision & number of queues, which are also set by defs file */
    hwCfg->numQueues = maxHwQs(CEDI_PdVar(cfg).regBase);

    reg = CPS_UncachedRead32(CEDI_RegAddr(revision_reg));

    hwCfg->moduleId  = EMAC_REGS__REVISION_REG__MODULE_IDENTIFICATION_NUMBER__READ(reg);
    hwCfg->moduleRev = EMAC_REGS__REVISION_REG__MODULE_REVISION__READ(reg);
    hwCfg->fixNumber =
    ...
    hwCfg->no_pcs = EMAC_REGS__DESIGNCFG_DEBUG1__NO_PCS__READ(reg);

    #ifdef EMAC_REGS__DESIGNCFG_DEBUG1__SERDES__READ
    hwCfg->serdes = EMAC_REGS__DESIGNCFG_DEBUG1__SERDES__READ(reg);
    #endif

    #ifdef EMAC_REGS__DESIGNCFG_DEBUG1__RDC_50__READ
    hwCfg->RDC_50 = EMAC_REGS__DESIGNCFG_DEBUG1__RDC_50__READ(reg);
    #endif

    #ifdef EMAC_REGS__DESIGNCFG_DEBUG1__TDC_50__READ
    hwCfg->TDC_50 = EMAC_REGS__DESIGNCFG_DEBUG1__TDC_50__READ(reg);
    #endif

    hwCfg->int_loopback = EMAC_REGS__DESIGNCFG_DEBUG1__INT_LOOPBACK__READ(reg);

    #ifdef EMAC_REGS__DESIGNCFG_DEBUG1__NO_INT_LOOPBACK__READ
    hwCfg->no_int_loopback = EMAC_REGS__DESIGNCFG_DEBUG1__NO_INT_LOOPBACK__READ(reg);
    #else
    hwCfg->no_int_loopback = !hwCfg->int_loopback;
    #endif

    hwCfg->ext_fifo_interface = EMAC_REGS__DESIGNCFG_DEBUG1__EXT_FIFO_INTERFACE__READ(reg);

    #ifdef EMAC_REGS__DESIGNCFG_DEBUG1__APB_REV1__READ
    hwCfg->apb_rev1 = EMAC_REGS__DESIGNCFG_DEBUG1__APB_REV1__READ(reg);
    #endif

    #ifdef EMAC_REGS__DESIGNCFG_DEBUG1__APB_REV2__READ
    hwCfg->apb_rev2 = EMAC_REGS__DESIGNCFG_DEBUG1__APB_REV2__READ(reg);
    #endif

    hwCfg->user_io        = EMAC_REGS__DESIGNCFG_DEBUG1__USER_IO__READ(reg);
    hwCfg->user_out_width = EMAC_REGS__DESIGNCFG_DEBUG1__USER_OUT_WIDTH__READ(reg);
    hwCfg->user_in_width  = EMAC_REGS__DESIGNCFG_DEBUG1__USER_IN_WIDTH__READ(reg);
    ...
    hwCfg->no_stats       = EMAC_REGS__DESIGNCFG_DEBUG1__NO_STATS__READ(reg);
    hwCfg->no_snapshot    = EMAC_REGS__DESIGNCFG_DEBUG1__NO_SNAPSHOT__READ(reg);
    hwCfg->irq_read_clear = EMAC_REGS__DESIGNCFG_DEBUG1__IRQ_READ_CLEAR__READ(reg);
    ...
    if (offloadsSupport(pD)) {
        hwCfg->exclude_cbs = EMAC_REGS__DESIGNCFG_DEBUG1__EXCLUDE_CBS__READ(reg);
    }
    #else
    hwCfg->exclude_cbs = 0;
    #endif
    ...
    hwCfg->axi_cache_value =
        EMAC_REGS__DESIGNCFG_DEBUG1__AXI_CACHE_VALUE__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug2));
    hwCfg->jumbo_max_length = EMAC_REGS__DESIGNCFG_DEBUG2__JUMBO_MAX_LENGTH__READ(reg);
    hwCfg->hprot_value      = EMAC_REGS__DESIGNCFG_DEBUG2__HPROT_VALUE__READ(reg);
    hwCfg->rx_pkt_buffer    = EMAC_REGS__DESIGNCFG_DEBUG2__RX_PKT_BUFFER__READ(reg);
    hwCfg->tx_pkt_buffer    = EMAC_REGS__DESIGNCFG_DEBUG2__TX_PKT_BUFFER__READ(reg);
    hwCfg->rx_pbuf_addr     = EMAC_REGS__DESIGNCFG_DEBUG2__RX_PBUF_ADDR__READ(reg);
    hwCfg->tx_pbuf_addr     = EMAC_REGS__DESIGNCFG_DEBUG2__TX_PBUF_ADDR__READ(reg);
    hwCfg->axi              = EMAC_REGS__DESIGNCFG_DEBUG2__AXI__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug3));
    hwCfg->num_spec_add_filters = EMAC_REGS__DESIGNCFG_DEBUG3__NUM_SPEC_ADD_FILTERS__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug5));
    hwCfg->rx_fifo_cnt_width    = EMAC_REGS__DESIGNCFG_DEBUG5__RX_FIFO_CNT_WIDTH__READ(reg);
    hwCfg->tx_fifo_cnt_width    = EMAC_REGS__DESIGNCFG_DEBUG5__TX_FIFO_CNT_WIDTH__READ(reg);
    hwCfg->tsu                  = EMAC_REGS__DESIGNCFG_DEBUG5__TSU__READ(reg);
    hwCfg->phy_ident            = EMAC_REGS__DESIGNCFG_DEBUG5__PHY_IDENT__READ(reg);
    hwCfg->dma_bus_width_def    = EMAC_REGS__DESIGNCFG_DEBUG5__DMA_BUS_WIDTH_DEF__READ(reg);
    hwCfg->mdc_clock_div        = EMAC_REGS__DESIGNCFG_DEBUG5__MDC_CLOCK_DIV__READ(reg);
    hwCfg->endian_swap_def      = EMAC_REGS__DESIGNCFG_DEBUG5__ENDIAN_SWAP_DEF__READ(reg);
    hwCfg->rx_pbuf_size_def     = EMAC_REGS__DESIGNCFG_DEBUG5__RX_PBUF_SIZE_DEF__READ(reg);
    hwCfg->tx_pbuf_size_def     = EMAC_REGS__DESIGNCFG_DEBUG5__TX_PBUF_SIZE_DEF__READ(reg);
    hwCfg->rx_buffer_length_def = EMAC_REGS__DESIGNCFG_DEBUG5__RX_BUFFER_LENGTH_DEF__READ(reg);
    hwCfg->tsu_clk              = EMAC_REGS__DESIGNCFG_DEBUG5__TSU_CLK__READ(reg);
    hwCfg->axi_prot_value       = EMAC_REGS__DESIGNCFG_DEBUG5__AXI_PROT_VALUE__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug6));
    hwCfg->tx_pbuf_queue_segment_size = EMAC_REGS__DESIGNCFG_DEBUG6__TX_PBUF_QUEUE_SEGMENT_SIZE__READ(reg);
    hwCfg->ext_tsu_timer       = EMAC_REGS__DESIGNCFG_DEBUG6__EXT_TSU_TIMER__READ(reg);
    hwCfg->tx_add_fifo_if      = EMAC_REGS__DESIGNCFG_DEBUG6__TX_ADD_FIFO_IF__READ(reg);
    hwCfg->host_if_soft_select = EMAC_REGS__DESIGNCFG_DEBUG6__HOST_IF_SOFT_SELECT__READ(reg);
    hwCfg->dma_addr_width      = EMAC_REGS__DESIGNCFG_DEBUG6__DMA_ADDR_WIDTH_IS_64B__READ(reg);
    hwCfg->pfc_multi_quantum   = EMAC_REGS__DESIGNCFG_DEBUG6__PFC_MULTI_QUANTUM__READ(reg);

    /* Offloads features: first available in GEM_GXL rev 1p07- */
    if (offloadsSupport(pD)) {
        #ifdef EMAC_REGS__DESIGNCFG_DEBUG6__PBUF_LSO__READ
        hwCfg->pbuf_lso = EMAC_REGS__DESIGNCFG_DEBUG6__PBUF_LSO__READ(reg);
        #else
        hwCfg->pbuf_lso = 0;
        #endif

        #ifdef EMAC_REGS__DESIGNCFG_DEBUG6__PBUF_RSC__READ
        hwCfg->pbuf_rsc = EMAC_REGS__DESIGNCFG_DEBUG6__PBUF_RSC__READ(reg);
        #else
        hwCfg->pbuf_rsc = 0;
        #endif
    ...
    hwCfg->tx_pbuf_num_segments_q0 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q0__READ(reg);
    hwCfg->tx_pbuf_num_segments_q1 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q1__READ(reg);
    hwCfg->tx_pbuf_num_segments_q2 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q2__READ(reg);
    hwCfg->tx_pbuf_num_segments_q3 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q3__READ(reg);
    hwCfg->tx_pbuf_num_segments_q4 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q4__READ(reg);
    hwCfg->tx_pbuf_num_segments_q5 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q5__READ(reg);
    hwCfg->tx_pbuf_num_segments_q6 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q6__READ(reg);
    hwCfg->tx_pbuf_num_segments_q7 = EMAC_REGS__DESIGNCFG_DEBUG7__TX_PBUF_NUM_SEGMENTS_Q7__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug8));
    hwCfg->num_type1_screeners   = EMAC_REGS__DESIGNCFG_DEBUG8__NUM_TYPE1_SCREENERS__READ(reg);
    hwCfg->num_type2_screeners   = EMAC_REGS__DESIGNCFG_DEBUG8__NUM_TYPE2_SCREENERS__READ(reg);
    hwCfg->num_scr2_ethtype_regs = EMAC_REGS__DESIGNCFG_DEBUG8__NUM_SCR2_ETHTYPE_REGS__READ(reg);
    hwCfg->num_scr2_compare_regs = EMAC_REGS__DESIGNCFG_DEBUG8__NUM_SCR2_COMPARE_REGS__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug9));
    hwCfg->tx_pbuf_num_segments_q8  = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q8__READ(reg);
    hwCfg->tx_pbuf_num_segments_q9  = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q9__READ(reg);
    hwCfg->tx_pbuf_num_segments_q10 = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q10__READ(reg);
    hwCfg->tx_pbuf_num_segments_q11 = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q11__READ(reg);
    hwCfg->tx_pbuf_num_segments_q12 = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q12__READ(reg);
    hwCfg->tx_pbuf_num_segments_q13 = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q13__READ(reg);
    hwCfg->tx_pbuf_num_segments_q14 = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q14__READ(reg);
    hwCfg->tx_pbuf_num_segments_q15 = EMAC_REGS__DESIGNCFG_DEBUG9__TX_PBUF_NUM_SEGMENTS_Q15__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug10));
    hwCfg->axi_access_pipeline_bits = EMAC_REGS__DESIGNCFG_DEBUG10__AXI_ACCESS_PIPELINE_BITS__READ(reg);
    hwCfg->rx_pbuf_data             = EMAC_REGS__DESIGNCFG_DEBUG10__RX_PBUF_DATA__READ(reg);
    hwCfg->tx_pbuf_data             = EMAC_REGS__DESIGNCFG_DEBUG10__TX_PBUF_DATA__READ(reg);
...
uint32_t emacProbe(CEDI_Config* config, CEDI_SysReq* sysReq) {
    ...
    regTmp  = EMAC_REGS__REVISION_REG__MODULE_IDENTIFICATION_NUMBER__READ(regVal);
    ...
    sysReq->privDataSize = sizeof(CEDI_PrivateData) + sizeof(uintptr_t) * (sumTxDesc + sumRxDesc);
...
uint32_t emacInit(void* pD, CEDI_Config const* config,
    ...
    if (!paramErr) {
        for (size_t i = 0; i < config->rxQs; i++) {
            if (config->rxQLen[i] > CEDI_MAX_RBQ_LENGTH) {
                paramErr = 1;
                vDbgMsg(DBG_GEN_MSG, 5,
                        "Error: out-of-range rxQLen(%zu) parameter\n", i);
                break;
            }
        }
    }
    ...
    if (!paramErr) {
        if ((config->txQAddr == (uintptr_t)NULL) || (config->txQPhyAddr == 0) || (config->rxQAddr == (uintptr_t)NULL) ||
            (config->rxQPhyAddr == 0)) {
            vDbgMsg(DBG_GEN_MSG, 5, "%s\n",
    ...
    if (!paramErr) {
        for (size_t i = 0; i < config->txQs; i++) {
            if (config->txQLen[i] > CEDI_MAX_TBQ_LENGTH) {
                paramErr = 1;
                vDbgMsg(DBG_GEN_MSG, 5,
                        "Error: out-of-range txQLen(%zu) parameter\n", i);
                break;
            }
        }
    }
    ...
    for (size_t i = 1; i <= CEDI_PdVar(hwCfg).num_spec_add_filters; i++) {
        emacDisableSpecAddr(pD, i);
    }

    /* and screener registers */
    for (size_t i = 0; i < CEDI_PdVar(hwCfg).num_type1_screeners - 1; i++) {
        emacSetType1ScreenReg(pD, i, &clrT1Scrn);
    }

    for (size_t i = 0; i < CEDI_PdVar(hwCfg).num_type2_screeners - 1; i++) {
...
#define CEDI_INT_STATUS_WR_CASE(Q) \
    case Q :                       \
        CPS_UncachedWrite32(CEDI_RegAddr(int_q##Q##_status), isrReg); \
        break;
    ...
    for (qNum = CEDI_PdVar(numQs) - 1; qNum < CEDI_MAX_TX_QUEUES; qNum--) {
        ...
        if (isrReg) {
            ...
            if (qNum == 0) {
                ...
                cond1 = 0;
                cond2 = EMAC_REGS__INT_Q_STATUS__RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION__READ(isrReg);
            ...
            /*************************** RxFrame *******************************/
            if (((qNum == 0) && (EMAC_REGS__INT_STATUS__RECEIVE_COMPLETE__READ(isrReg))) ||
                ((qNum > 0) && (EMAC_REGS__INT_Q_STATUS__RECEIVE_COMPLETE__READ(isrReg)))) {
                /*vDbgMsg(DBG_GEN_MSG, 10,
            ...
            /************************ HResp not OK Event ***********************/
            if (((qNum == 0) && (EMAC_REGS__INT_STATUS__RESP_NOT_OK__READ(isrReg))) ||
                ((qNum  > 0) && (EMAC_REGS__INT_Q_STATUS__RESP_NOT_OK__READ(isrReg)))) {
                vDbgMsg(DBG_GEN_MSG, 10,
            ...
            if ((qNum == 0) &&
                ...
                CEDI_PdVar(anStatus).duplexRes  = EMAC_REGS__NETWORK_STATUS__MAC_FULL_DUPLEX__READ(regVal);
                CEDI_PdVar(anStatus).linkState  = EMAC_REGS__NETWORK_STATUS__PCS_LINK_STATE__READ(regVal);
                CEDI_PdVar(anStatus).pauseRxRes = EMAC_REGS__NETWORK_STATUS__MAC_PAUSE_RX_EN__READ(regVal);
                CEDI_PdVar(anStatus).pauseTxRes = EMAC_REGS__NETWORK_STATUS__MAC_PAUSE_TX_EN__READ(regVal);
            ...
            if (qNum == 0) {
                ...
                if (cond1 || cond2) {
                    ...
                    vDbgMsg(DBG_GEN_MSG, 10, "EMAC (0x%08X) PTP Primary Frame Tx, type:0x%08X\n",
                            (uint32_t)CEDI_PdVar(cfg).regBase, events);
            ...
            if (qNum == 0) {
                ...
                if (cond1 || cond2) {
                    ...
                    vDbgMsg(DBG_GEN_MSG, 10, "EMAC (0x%08X) PTP Peer Frame Tx, type:0x%08X\n",
                            (uint32_t)CEDI_PdVar(cfg).regBase, events);
            ...
            if (qNum == 0) {
                ...
                if (cond1 || cond2) {
                    ...
                    vDbgMsg(DBG_GEN_MSG, 10, "EMAC (0x%08X) PTP Primary Frame Rx, type:0x%08X\n",
                            (uint32_t)CEDI_PdVar(cfg).regBase, events);
            ...
            if (qNum == 0) {
                ...
                if (cond1 || cond2) {
                    ...
                    vDbgMsg(DBG_GEN_MSG, 10, "EMAC (0x%08X) PTP Peer Frame Rx, type:0x%08X\n",
                            (uint32_t)CEDI_PdVar(cfg).regBase, events);
            ...
            /************************* LPI Status Change ***********************/
            if ((qNum == 0) && EMAC_REGS__INT_STATUS__RECEIVE_LPI_INDICATION_STATUS_BIT_CHANGE__READ(isrReg)) {
...
uint32_t emacSetEventEnable(void* pD, uint32_t events, uint8_t enable,
                            uint8_t queueNum) {
    #define CEDI_ENABLE_INT(Q) \
    if ((queueNum == Q) || (queueNum == CEDI_ALL_QUEUES)) \
        CPS_UncachedWrite32(CEDI_RegAddr(int_q##Q##_enable), regVal);

    #define CEDI_DISABLE_INT(Q) \
    if ((queueNum == Q) || (queueNum == CEDI_ALL_QUEUES)) \
    ...
    if ((events & (~(((queueNum == 0) || (queueNum == CEDI_ALL_QUEUES)) ? CEDI_EVSET_ALL_Q0_EVENTS
                                                                        : CEDI_EVSET_ALL_QN_EVENTS))) != 0) {
        vDbgMsg(DBG_GEN_MSG, 5,
        ...
        if (enable) {
            ...
            if ((queueNum == 0) || (queueNum == CEDI_ALL_QUEUES)) {
                ...
                if (events & CEDI_EV_TX_UNDERRUN) {
                    EMAC_REGS__INT_ENABLE__ENABLE_TRANSMIT_BUFFER_UNDER_RUN_INTERRUPT__SET(regVal);
                }

                if (events & CEDI_EV_TX_RETRY_EX_LATE_COLL) {
                    EMAC_REGS__INT_ENABLE__ENABLE_RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_INTERRUPT__SET(regVal);
                }
                ...
                if (events & CEDI_EV_PCS_AN_COMPLETE) {
                    EMAC_REGS__INT_ENABLE__ENABLE_PCS_AUTO_NEGOTIATION_COMPLETE_INTERRUPT__SET(regVal);
                }
                ...
                if (events & CEDI_EV_TX_FR_CORRUPT) {
                    EMAC_REGS__INT_ENABLE__ENABLE_TRANSMIT_FRAME_CORRUPTION_DUE_TO_AMBA_ERROR_INTERRUPT__SET(regVal);
                }
                ...
                if (events & CEDI_EV_PAUSE_NZ_QU_RX) {
                    EMAC_REGS__INT_ENABLE__ENABLE_PAUSE_FRAME_WITH_NON_ZERO_PAUSE_QUANTUM_INTERRUPT__SET(regVal);
                }
            ...
            if (queueNum > 0) {
                ...
                if (events & CEDI_EV_TX_RETRY_EX_LATE_COLL) {
                    EMAC_REGS__INT_Q_ENABLE__ENABLE_RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_INTERRUPT__SET(regVal);
                }

                if (events & CEDI_EV_TX_FR_CORRUPT) {
                    EMAC_REGS__INT_Q_ENABLE__ENABLE_TRANSMIT_FRAME_CORRUPTION_DUE_TO_AMBA_ERROR_INTERRUPT__SET(regVal);
                }
            ...
            if ((queueNum == 0) || (queueNum == CEDI_ALL_QUEUES)) {
                ...
                if (events & CEDI_EV_TX_UNDERRUN) {
                    EMAC_REGS__INT_DISABLE__DISABLE_TRANSMIT_BUFFER_UNDER_RUN_INTERRUPT__SET(regVal);
                }

                if (events & CEDI_EV_TX_RETRY_EX_LATE_COLL) {
                    EMAC_REGS__INT_DISABLE__DISABLE_RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_INTERRUPT__SET(regVal);
                }
                ...
                if (events & CEDI_EV_PCS_AN_COMPLETE) {
                    EMAC_REGS__INT_DISABLE__DISABLE_PCS_AUTO_NEGOTIATION_COMPLETE_INTERRUPT__SET(regVal);
                }
                ...
                if (events & CEDI_EV_TX_FR_CORRUPT) {
                    EMAC_REGS__INT_DISABLE__DISABLE_TRANSMIT_FRAME_CORRUPTION_DUE_TO_AMBA_ERROR_INTERRUPT__SET(regVal);
                }
                ...
                if (events & CEDI_EV_PAUSE_NZ_QU_RX) {
                    EMAC_REGS__INT_DISABLE__DISABLE_PAUSE_FRAME_WITH_NON_ZERO_PAUSE_QUANTUM_INTERRUPT__SET(regVal);
                }
            ...
            if (queueNum > 0) {
                ...
                if (events & CEDI_EV_TX_RETRY_EX_LATE_COLL) {
                    EMAC_REGS__INT_Q_DISABLE__DISABLE_RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_INTERRUPT__SET(regVal);
                }
...
uint32_t emacGetEventEnable(void* pD, uint8_t queueNum, uint32_t* event) {
    #define CEDI_READ_INT_MASK_CASE(Q)  \
    case Q :                            \
        regVal = ~CPS_UncachedRead32(CEDI_RegAddr(int_q##Q##_mask)); \
        break;
    ...
    if (queueNum == 0) {
        ...
        if (regVal) {
            ...
            if (EMAC_REGS__INT_MASK__RECEIVE_USED_BIT_READ_INTERRUPT_MASK__READ(regVal)) {
                ret |= CEDI_EV_RX_USED_READ;
            }

            if (EMAC_REGS__INT_MASK__TRANSMIT_USED_BIT_READ_INTERRUPT_MASK__READ(regVal)) {
                ret |= CEDI_EV_TX_USED_READ;
            }

            if (EMAC_REGS__INT_MASK__TRANSMIT_BUFFER_UNDER_RUN_INTERRUPT_MASK__READ(regVal)) {
                ret |= CEDI_EV_TX_UNDERRUN;
            }

            if (EMAC_REGS__INT_MASK__RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_MASK__READ(regVal)) {
            ...
            if (EMAC_REGS__INT_MASK__PCS_AUTO_NEGOTIATION_COMPLETE_INTERRUPT_MASK__READ(regVal)) {
            ...
            if (EMAC_REGS__INT_MASK__TSU_SECONDS_REGISTER_INCREMENT_MASK__READ(regVal)) {
            ...
            if (EMAC_REGS__INT_MASK__PAUSE_FRAME_WITH_NON_ZERO_PAUSE_QUANTUM_INTERRUPT_MASK__READ(regVal)) {
            ...
            if (EMAC_REGS__INT_MASK__PAUSE_FRAME_TRANSMITTED_INTERRUPT_MASK__READ(regVal)) {
            ...
            if (EMAC_REGS__INT_MASK__PTP_DELAY_REQ_FRAME_TRANSMITTED_MASK__READ(regVal)) {
            ...
            if (EMAC_REGS__INT_MASK__PTP_PDELAY_REQ_FRAME_RECEIVED_MASK__READ(regVal)) {
                ret |= CEDI_EV_PTP_RX_PDLY_REQ;
            }

            if (EMAC_REGS__INT_MASK__PTP_PDELAY_RESP_FRAME_RECEIVED_MASK__READ(regVal)) {
                ret |= CEDI_EV_PTP_RX_PDLY_RSP;
            }

            if (EMAC_REGS__INT_MASK__PTP_PDELAY_REQ_FRAME_TRANSMITTED_MASK__READ(regVal)) {
                ret |= CEDI_EV_PTP_TX_PDLY_REQ;
            }

            if (EMAC_REGS__INT_MASK__PTP_PDELAY_RESP_FRAME_TRANSMITTED_MASK__READ(regVal)) {
        ...
        if (regVal) {
            if (EMAC_REGS__INT_Q_MASK__RECEIVE_COMPLETE_INTERRUPT_MASK__READ(regVal)) {
                ret |= CEDI_EV_RX_COMPLETE;
            }

            if (EMAC_REGS__INT_Q_MASK__RX_USED_INTERRUPT_MASK__READ(regVal)) {
                ret |= CEDI_EV_RX_USED_READ;
            }

            if (EMAC_REGS__INT_Q_MASK__RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_INTERRUPT_MASK__READ(regVal)) {
                ret |= CEDI_EV_TX_RETRY_EX_LATE_COLL;
            }
            ...
            if (EMAC_REGS__INT_Q_MASK__TRANSMIT_COMPLETE_INTERRUPT_MASK__READ(regVal)) {
                ret |= CEDI_EV_TX_COMPLETE;
            }
...
uint32_t emacSetIntrptModerate(void* pD, uint8_t txIntDelay, uint8_t rxIntDelay) {
    uint32_t reg;
...
uint32_t emacGetIntrptModerate(void* pD, uint8_t* txIntDelay, uint8_t* rxIntDelay) {
    uint32_t reg;
...
uint32_t emacSetRxDmaDataAddrMask(void* pD, uint8_t enableBit, uint8_t bitValues) {
    uint32_t reg = 0;

    if ((enableBit > 0xf) || (bitValues > 0xf)) {
        return EINVAL;
    }
...
uint32_t emacGetRxDmaDataAddrMask(void* pD, uint8_t* enableBit, uint8_t* bitValues) {
    uint32_t reg;
...
uint32_t emacSetTxPauseQuantum(void* pD, uint16_t value, uint8_t qpriority) {
    ...
    if ((CEDI_PdVar(hwCfg).pfc_multi_quantum == 0) && (qpriority > 0)) {
        return ENOTSUP;
    }
...
uint32_t emacSetLoopback(void* pD, uint8_t mode) {
    ...
    if ((CEDI_PdVar(hwCfg).no_pcs && (mode == CEDI_SERDES_LOOPBACK)) ||
        (CEDI_PdVar(hwCfg).no_int_loopback && (mode == CEDI_LOCAL_LOOPBACK))) {
        return ENOTSUP;
    }
...
uint32_t emacSet1588TimerInc(void* pD, CEDI_TimerIncrement* incSettings) {
    ...
    reg = 0;
    EMAC_REGS__TSU_TIMER_INCR__NS_INCREMENT__MODIFY(reg, incSettings->nanoSecsInc);
    EMAC_REGS__TSU_TIMER_INCR__ALT_NS_INCR__MODIFY(reg, incSettings->altNanoSInc);
    ...
    #else
    reg = 0;
    EMAC_REGS__TSU_TIMER_INCR_SUB_NSEC__TIMER__MODIFY(reg, incSettings->subNsInc);
    CPS_UncachedWrite32(CEDI_RegAddr(tsu_timer_incr_sub_nsec), reg);

    reg = 0;
    EMAC_REGS__TSU_TIMER_INCR__COUNT__MODIFY(reg, incSettings->nanoSecsInc);
    EMAC_REGS__TSU_TIMER_INCR__ALT_COUNT__MODIFY(reg, incSettings->altNanoSInc);
    EMAC_REGS__TSU_TIMER_INCR__NUM_INCS__MODIFY(reg, incSettings->altIncCount);
    CPS_UncachedWrite32(CEDI_RegAddr(tsu_timer_incr), reg);
    #endif
...
uint32_t emacGet1588TimerInc(void* pD, CEDI_TimerIncrement* incSettings) {
    ...
    incSettings->subNsInc = EMAC_REGS__TSU_TIMER_INCR_SUB_NSEC__SUB_NS_INCR__READ(reg);
    if (subNsTsuInc24bSupport(pD)) {
        incSettings->lsbSubNsInc = EMAC_REGS__TSU_TIMER_INCR_SUB_NSEC__SUB_NS_INCR_LSB__READ(reg);
    }
    else {
    ...
    incSettings->nanoSecsInc = EMAC_REGS__TSU_TIMER_INCR__NS_INCREMENT__READ(reg);
    incSettings->altNanoSInc = EMAC_REGS__TSU_TIMER_INCR__ALT_NS_INCR__READ(reg);
    incSettings->altIncCount = EMAC_REGS__TSU_TIMER_INCR__NUM_INCS__READ(reg);
    #else
    incSettings->subNsInc =
        EMAC_REGS__TSU_TIMER_INCR_SUB_NSEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_timer_incr_sub_nsec)));
    incSettings->lsbSubNsInc = 0;
    reg                      = CPS_UncachedRead32(CEDI_RegAddr(tsu_timer_incr));
...
uint32_t emacGetTsuTimerCompVal(void* pD, CEDI_TsuTimerVal* time) {
    ...
    time->nanosecs  = EMAC_REGS__TSU_NSEC_CMP__COMPARISON_VALUE__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_nsec_cmp)));
    time->secsLower = EMAC_REGS__TSU_SEC_CMP__COMPARISON_VALUE__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_sec_cmp)));
...
uint32_t emacGetPtpFrameTxTime(void* pD, CEDI_1588TimerVal* time) {
    ...
    time->nanosecs  = EMAC_REGS__TSU_PTP_TX_NSEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_ptp_tx_nsec)));
    time->secsLower = EMAC_REGS__TSU_PTP_TX_SEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_ptp_tx_sec)));
...
uint32_t emacGetPtpFrameRxTime(void* pD, CEDI_1588TimerVal* time) {
    ...
    time->nanosecs  = EMAC_REGS__TSU_PTP_RX_NSEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_ptp_rx_nsec)));
    time->secsLower = EMAC_REGS__TSU_PTP_RX_SEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_ptp_rx_sec)));
    time->secsUpper =
        EMAC_REGS__TSU_PTP_RX_MSB_SEC__TIMER_SECONDS__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_ptp_rx_msb_sec)));
    return 0;
}
...
uint32_t emacGetPtpPeerFrameTxTime(void* pD, CEDI_1588TimerVal* time) {
    ...
    time->nanosecs  = EMAC_REGS__TSU_PEER_TX_NSEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_peer_tx_nsec)));
    time->secsLower = EMAC_REGS__TSU_PEER_TX_SEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_peer_tx_sec)));
    time->secsUpper =
        EMAC_REGS__TSU_PEER_TX_MSB_SEC__TIMER_SECONDS__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_peer_tx_msb_sec)));
    return 0;
}
...
uint32_t emacGetPtpPeerFrameRxTime(void* pD, CEDI_1588TimerVal* time) {
    ...
    time->nanosecs  = EMAC_REGS__TSU_PEER_RX_NSEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_peer_rx_nsec)));
    time->secsLower = EMAC_REGS__TSU_PEER_RX_SEC__TIMER__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_peer_rx_sec)));
    time->secsUpper =
        EMAC_REGS__TSU_PEER_RX_MSB_SEC__TIMER_SECONDS__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_peer_rx_msb_sec)));
    return 0;
}
...
uint32_t emacGet1588SyncStrobeTime(void* pD, CEDI_1588TimerVal* time) {
    ...
    time->nanosecs  = EMAC_REGS__TSU_STROBE_NSEC__STROBE__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_strobe_nsec)));
    time->secsLower = EMAC_REGS__TSU_STROBE_SEC__STROBE__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_strobe_sec)));
    time->secsUpper = EMAC_REGS__TSU_STROBE_MSB_SEC__STROBE__READ(CPS_UncachedRead32(CEDI_RegAddr(tsu_strobe_msb_sec)));
    return 0;
}
...
uint32_t emacGetStoreRxTimeStamp(void* pD, uint8_t* enable) {
    ...
    *enable = EMAC_REGS__NETWORK_CONTROL__STORE_RX_TS__READ(CPS_UncachedRead32(CEDI_RegAddr(network_control)));
    return 0;
}
...
uint32_t emacStartAutoNegotiation(void* pD, CEDI_AnAdvPage* advDat) {
    ...
    if ((advDat->fullDuplex > 1) || (advDat->halfDuplex > 1) || (advDat->nextPage > 1) ||
        (advDat->pauseCap > CEDI_AN_PAUSE_CAP_BOTH) || (advDat->remFlt > CEDI_AN_REM_FLT_AN_ERR)) {
        return EINVAL;
    }
...
uint32_t emacSetAutoNegEnable(void* pD, uint8_t enable) {
    ...
    if (CEDI_PdVar(hwCfg).no_pcs ||
        (EMAC_REGS__NETWORK_CONFIG__SGMII_MODE_ENABLE__READ(CPS_UncachedRead32(CEDI_RegAddr(network_config))))) {
        return ENOTSUP;
    }
...
uint32_t emacGetLpAbilityPage(void* pD, CEDI_LpAbilityPage* lpAbl) {
    ...
    if (EMAC_REGS__NETWORK_CONFIG__SGMII_MODE_ENABLE__READ(
        ...
        /* SGMII mode format */
        lpAbl->sgmii                       = 1;
        lpAbl->ablInfo.sgmLpAbl.speed      = (CEDI_IfSpeed)(EMAC_REGS__PCS_AN_LP_BASE__SPEED_RESERVED__READ(reg) >> 1);
        lpAbl->ablInfo.sgmLpAbl.lpAck      = EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_ACKNOWLEDGE__READ(reg);
        lpAbl->ablInfo.sgmLpAbl.linkStatus = EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_NEXT_PAGE_STATUS__READ(reg);
        lpAbl->ablInfo.sgmLpAbl.duplex     = EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_REMOTE_FAULT_DUPLEX_MODE__READ(reg);
    }
    else {
        /* Default format */
        lpAbl->sgmii                       = 0;
        lpAbl->ablInfo.defLpAbl.fullDuplex = EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_FULL_DUPLEX__READ(reg);
        lpAbl->ablInfo.defLpAbl.halfDuplex = EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_HALF_DUPLEX__READ(reg);
        lpAbl->ablInfo.defLpAbl.pauseCap   = (CEDI_PauseCap)EMAC_REGS__PCS_AN_LP_BASE__PAUSE__READ(reg);
        lpAbl->ablInfo.defLpAbl.lpAck      = EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_ACKNOWLEDGE__READ(reg);
        lpAbl->ablInfo.defLpAbl.remFlt =
            (CEDI_RemoteFault)EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_REMOTE_FAULT_DUPLEX_MODE__READ(reg);
        lpAbl->ablInfo.defLpAbl.lpNextPage = EMAC_REGS__PCS_AN_LP_BASE__LINK_PARTNER_NEXT_PAGE_STATUS__READ(reg);
    }
...
uint32_t emacGetPageRx(void* pD) {
    ...
    return EMAC_REGS__PCS_AN_EXP__PAGE_RECEIVED__READ(CPS_UncachedRead32(CEDI_RegAddr(pcs_an_exp)));
}
...
uint32_t emacSetNextPageTx(void* pD, CEDI_AnNextPage* npDat) {
    ...
    if ((npDat->message > 0x7FF) || (npDat->ack2 > 1) || (npDat->msgPage > 1) || (npDat->np > 1)) {
        return EINVAL;
    }
...
uint32_t emacGetMdioEnable(void* pD) {
    return EMAC_REGS__NETWORK_CONTROL__MAN_PORT_EN__READ(CPS_UncachedRead32(
                CEDI_RegAddr(network_control)));
}
 ...
 *           - enum CEDI_MdioDevType is available to specify the device type
 */
void emacPhyStartMdioRead(void* pD, uint8_t flags, uint8_t phyAddr, uint8_t devReg) {
    uint32_t reg;
    ...
    if (flags & CEDI_MDIO_FLG_CLAUSE_45) {
        if (flags & CEDI_MDIO_FLG_INC_ADDR) {
            EMAC_REGS__PHY_MANAGEMENT__OPERATION__MODIFY(reg, CEDI_PHY_CL45_READ_INC_OP);
        }
        else {
            EMAC_REGS__PHY_MANAGEMENT__OPERATION__MODIFY(reg, CEDI_PHY_CL45_READ_OP);
        }
    }
    else {
...
uint32_t emacReadStats(void* pD) {
    ...
    stats->fr1024_1518byteTx = CPS_UncachedRead32(CEDI_RegAddr(frames_txed_1024));
    stats->fr1519_byteTx     = CPS_UncachedRead32(CEDI_RegAddr(frames_txed_1519));
    stats->underrunFrTx      = EMAC_REGS__TX_UNDERRUNS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(tx_underruns)));
    stats->singleCollFrTx    = CPS_UncachedRead32(CEDI_RegAddr(single_collisions));
    stats->multiCollFrTx     = CPS_UncachedRead32(CEDI_RegAddr(multiple_collisions));
    ...
    stats->fr512_1023byteRx  = CPS_UncachedRead32(CEDI_RegAddr(frames_rxed_512));
    stats->fr1024_1518byteRx = CPS_UncachedRead32(CEDI_RegAddr(frames_rxed_1024));
    stats->fr1519_byteRx     = CPS_UncachedRead32(CEDI_RegAddr(frames_rxed_1519));
    stats->undersizeFrRx = EMAC_REGS__UNDERSIZE_FRAMES__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(undersize_frames)));
    stats->oversizeFrRx  = EMAC_REGS__EXCESSIVE_RX_LENGTH__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(excessive_rx_length)));
    stats->jabbersRx     = EMAC_REGS__RX_JABBERS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_jabbers)));
    stats->fcsErrorsRx   = EMAC_REGS__FCS_ERRORS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(fcs_errors)));
    stats->lenChkErrRx   = EMAC_REGS__RX_LENGTH_ERRORS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_length_errors)));
    stats->rxSymbolErrs  = EMAC_REGS__RX_SYMBOL_ERRORS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_symbol_errors)));
    stats->rxResourcErrs = CPS_UncachedRead32(CEDI_RegAddr(rx_resource_errors));
    stats->overrunFrRx   = EMAC_REGS__RX_OVERRUNS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_overruns)));
    stats->ipChksumErrs  = EMAC_REGS__RX_IP_CK_ERRORS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_ip_ck_errors)));
    stats->tcpChksumErrs = EMAC_REGS__RX_TCP_CK_ERRORS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_tcp_ck_errors)));
    stats->udpChksumErrs = EMAC_REGS__RX_UDP_CK_ERRORS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_udp_ck_errors)));
    stats->dmaRxPBufFlush =
        EMAC_REGS__AUTO_FLUSHED_PKTS__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(auto_flushed_pkts)));

    /*
        vDbgMsg(DBG_GEN_MSG, 10, "Non-zero Error Stats: %s%u %s%u %s%u %s%u %s%u %s%u %s%u %s%u %s%u %s%u %s%u %s%u %s%u
...
uint32_t emacSetWakeOnLanReg(void* pD, CEDI_WakeOnLanReg* regVals) {
    ...
    if ((pD == NULL) || (regVals == NULL) || (regVals->magPktEn > 1) || (regVals->arpEn > 1) ||
        (regVals->specAd1En > 1) || (regVals->multiHashEn > 1)) {
        return EINVAL;
    }
...
uint32_t emacGetLpiStats(void* pD, CEDI_LpiStats* lpiStats) {
    ...
    lpiStats->rxLpiTrans = EMAC_REGS__RX_LPI__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_lpi)));
    lpiStats->rxLpiTime  = EMAC_REGS__RX_LPI_TIME__LPI_TIME__READ(CPS_UncachedRead32(CEDI_RegAddr(rx_lpi_time)));
    lpiStats->txLpiTrans = EMAC_REGS__TX_LPI__COUNT__READ(CPS_UncachedRead32(CEDI_RegAddr(tx_lpi)));
    lpiStats->txLpiTime  = EMAC_REGS__TX_LPI_TIME__LPI_TIME__READ(CPS_UncachedRead32(CEDI_RegAddr(tx_lpi_time)));
    return 0;
}
...
uint32_t emacWriteUserOutputs(void* pD, uint16_t outVal) {
    ...
    tmp = 0;
    EMAC_REGS__USER_IO_REGISTER__USER_PROGRAMMABLE_OUTPUTS__MODIFY(tmp, outVal);
...
uint32_t emacSetUserOutPin(void* pD, uint8_t pin, uint8_t state) {
    ...
    EMAC_REGS__USER_IO_REGISTER__USER_PROGRAMMABLE_OUTPUTS__MODIFY(reg, val);
...
CEDI_OBJ const* CEDI_GetInstance(void) {
    return &EmacDrv;
}
