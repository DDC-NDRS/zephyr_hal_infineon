...
uint32_t emacNumRxUsed(void* pD, uint8_t queueNum) {
    ...
    for (tmp = 0; tmp < rxQ->numRxBufs; tmp++) {
        ...
        if (thisWd & CEDI_RXD_WRAP) {
            ...
            thisDesc = (rxDesc*)(((uintptr_t)(thisDesc)) + (CEDI_PdVar(rxDescriptorSize)));
        }
    }
...
#pragma clang optimize on
    ...
    if (descWd0 & CEDI_RXD_USED) {
        ...
        /* upper 32 bits if 64 bit addressing */
        if (CEDI_PdVar(cfg).dmaAddrBusWidth) {
            oldbuf.pAddr |= ((uint64_t)CPS_UncachedRead32(&rxQ->rxDescTail->word[2])) << 32;
        }
        #endif
        ...
        *(rxQ->rxStopVA) = buf->vAddr;
        ...
        if ((CEDI_PdVar(cfg).enRxExtBD) && (descWd0 & CEDI_RXD_TS_VALID)) {
            ...
            descData->rxTsData.tsSecs =
                (((tsUpperWd & CEDI_TS_SEC1_MASK) << CEDI_TS_SEC1_POS_SHIFT) | (tsLowerWd >> CEDI_TS_SEC0_SHIFT));
...
void emacGetRxDescStat(void* pD, uint32_t rxDStatWord, CEDI_RxDescStat* rxDStat) {
    ...
    if (rxDStat->vlanTagDet) {
        rxDStat->vlanPri = (wd1 & CEDI_RXD_VLAN_PRI_MASK) >> CEDI_RXD_VLAN_PRI_SHIFT;
    }
    else {
        rxDStat->vlanPri = 0;
    }
    ...
    if (EMAC_REGS__NETWORK_CONFIG__RECEIVE_CHECKSUM_OFFLOAD_ENABLE__READ(reg)) {
        rxDStat->chkOffStat    = (wd1 & CEDI_RXD_TYP_IDR_CHK_STA_MASK) >> CEDI_RXD_TYP_IDR_CHK_STA_SHIFT;
        ...
        rxDStat->typeMatchReg  = (wd1 & CEDI_RXD_TYP_IDR_CHK_STA_MASK) >> CEDI_RXD_TYP_IDR_CHK_STA_SHIFT;
        rxDStat->typeIdMatch   = (wd1 & CEDI_RXD_TYP_MAT_SNP_NCFI) ? 1 : 0;
    }

    rxDStat->specAddReg = (wd1 & CEDI_RXD_SPEC_REG_MASK) >> CEDI_RXD_SPEC_REG_SHIFT;
    if (CEDI_PdVar(hwCfg).rx_pkt_buffer &&
        (CEDI_PdVar(hwCfg).num_spec_add_filters > 4)) { /* extra spec. addr matching variation */
...
uint32_t emacRemoveRxBuf(void* pD, uint8_t queueNum, CEDI_BuffAddr* buf) {
    ...
    buf->pAddr = CPS_UncachedRead32(&(rxQ->rxDescEnd->word[0])) & CEDI_RXD_ADDR_MASK;
    /* get virtual address */
...
uint32_t emacResetRxQ(void* pD, uint8_t queueNum, uint8_t ptrsOnly) {
    #define CEDI_WR_RXQ_PTR_REG_N_CASE(Q)   \
    case Q :                                \
...
#pragma clang optimize on
    ...
    if ((queueNum >= CEDI_PdVar(cfg).rxQs) || (ptrsOnly > 1)) {
        return EINVAL;
    }
        ...
        if (ptrsOnly) {
            if (rxQ->rxDescStop == rxQ->rxDescEnd) {
                ...
                *(rxQ->rxStopVA) = 0;
                ...
                CPS_UncachedWrite32(&(rxQ->rxDescStop->word[1]), CEDI_RXD_EMPTY);
                /* inc stop pointer */
                rxQ->rxDescStop = (rxDesc*)(((uintptr_t)rxQ->rxDescStop) + (CEDI_PdVar(rxDescriptorSize)));
                /* inc VA stop pointer */
                rxQ->rxStopVA++;
            }
        }
...
uint32_t emacGetRxStatus(void* pD, CEDI_RxStatus* status) {
    ...
    status->buffNotAvail = EMAC_REGS__RECEIVE_STATUS__BUFFER_NOT_AVAILABLE__READ(reg);
    status->frameRx      = EMAC_REGS__RECEIVE_STATUS__FRAME_RECEIVED__READ(reg);
    status->rxOverrun    = EMAC_REGS__RECEIVE_STATUS__RECEIVE_OVERRUN__READ(reg);
    status->hRespNotOk   = EMAC_REGS__RECEIVE_STATUS__RESP_NOT_OK__READ(reg);
...
uint32_t emacGetRscEnable(void* pD, uint8_t queue, uint8_t* enable) {
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(rsc_control));
    *enable = (EMAC_REGS__RSC_CONTROL__RSC_CONTROL__READ(reg) & (1 << (queue - 1))) ? 1 : 0;
...
uint32_t emacSetRxPartialStFwd(void* pD, uint32_t watermark, uint8_t enable) {
    ...
    if (enable) {
        EMAC_REGS__PBUF_RXCUTTHRU__DMA_RX_CUTTHRU_THRESHOLD__MODIFY(reg, watermark);
...
uint32_t emacSetSpecificAddr(void* pD, uint8_t addrNum, CEDI_MacAddress* addr,
    ...
    case (reg) :                                            \
        ...
        CPS_UncachedWrite32((CEDI_RegAddr(spec_add##reg##_top)), regVal); \
        break;
...
uint32_t emacGetSpecificAddr(void* pD, uint8_t addrNum, CEDI_MacAddress* addr,
    ...
    case (reg) :                                            \
        ...
        if (reg == 1) {                                     \
            regAddrTop      = EMAC_REGS__SPEC_ADD_TOP_NO_MASK__ADDRESS__READ(regTopVal);     \
            *specFilterType = EMAC_REGS__SPEC_ADD_TOP_NO_MASK__FILTER_TYPE__READ(regTopVal); \
            *byteMask       = 0;                            \
        }                                                   \
        else {                                              \
            regAddrTop      = EMAC_REGS__SPEC_ADD_TOP__ADDRESS__READ(regTopVal);          \
            *specFilterType = EMAC_REGS__SPEC_ADD_TOP__FILTER_TYPE__READ(regTopVal);      \
            *byteMask       = EMAC_REGS__SPEC_ADD_TOP__FILTER_BYTE_MASK__READ(regTopVal); \
        }                                                   \
        break;
...
uint32_t emacSetSpecificAddr1Mask(void* pD, CEDI_MacAddress* mask) {
    ...
    EMAC_REGS__MASK_ADD1_BOTTOM__ADDRESS_MASK__MODIFY(reg,
        mask->byte[0] + (mask->byte[1] << 8) + (mask->byte[2] << 16) + (mask->byte[3] << 24));
...
uint32_t emacDisableSpecAddr(void* pD, uint8_t addrNum) {
    ...
    case (reg) :                                            \
        CPS_UncachedWrite32((CEDI_RegAddr(spec_add##reg##_bottom)), 0); \
        break;
 ...
 */
uint32_t emacGetTypeIdMatch(void* pD, uint8_t matchSel, uint16_t* typeId, uint8_t* enabled) {
    uint32_t regVal = 0;

    if ((matchSel < 1) || (matchSel > 4) || (enabled == NULL)) {
        return EINVAL;
    }
...
uint32_t emacSetType2ScreenReg(void* pD, uint8_t regNum, CEDI_T2Screen* regVals) {
    ...
    case (rNum) :                                               \
        CPS_UncachedWrite32(CEDI_RegAddr(screening_type_2_register_##rNum), reg); \
        break;
    ...
    if ((regNum >= CEDI_PdVar(hwCfg).num_type2_screeners) ||
        ...
        ((regVals->eTypeEnable) && (regVals->ethTypeIndex >= CEDI_PdVar(hwCfg).num_scr2_ethtype_regs)) ||
        (regVals->compAEnable > 1) ||
        ((regVals->compAEnable) && (regVals->compAIndex >= 32)) ||
        ((regVals->compAEnable) && (regVals->compAIndex >= CEDI_PdVar(hwCfg).num_scr2_compare_regs)) ||
        (regVals->compBEnable > 1) ||
        ((regVals->compBEnable) && (regVals->compBIndex >= 32)) ||
        ((regVals->compBEnable) && (regVals->compBIndex >= CEDI_PdVar(hwCfg).num_scr2_compare_regs)) ||
        (regVals->compCEnable > 1) ||
        ((regVals->compCEnable) && (regVals->compCIndex >= 32)) ||
        ((regVals->compCEnable) && (regVals->compCIndex >= CEDI_PdVar(hwCfg).num_scr2_compare_regs))) {
        return EINVAL;
    }

    reg = 0;
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__QUEUE_NUMBER__MODIFY(reg, regVals->qNum);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__VLAN_ENABLE__MODIFY(reg, regVals->vlanEnable);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__VLAN_PRIORITY__MODIFY(reg, regVals->vlanPriority);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__ETHERTYPE_ENABLE__MODIFY(reg, regVals->eTypeEnable);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__INDEX__MODIFY(reg, regVals->ethTypeIndex);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_A_ENABLE__MODIFY(reg, regVals->compAEnable);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_A__MODIFY(reg, regVals->compAIndex);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_B_ENABLE__MODIFY(reg, regVals->compBEnable);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_B__MODIFY(reg, regVals->compBIndex);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_C_ENABLE__MODIFY(reg, regVals->compCEnable);
    EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_C__MODIFY(reg, regVals->compCIndex);
...
uint32_t emacGetType2ScreenReg(void* pD, uint8_t regNum, CEDI_T2Screen* regVals) {
    ...
    regVals->qNum         = EMAC_REGS__SCREENING_TYPE_2_REGISTER__QUEUE_NUMBER__READ(reg);
    regVals->vlanPriority = EMAC_REGS__SCREENING_TYPE_2_REGISTER__VLAN_PRIORITY__READ(reg);
    regVals->vlanEnable   = EMAC_REGS__SCREENING_TYPE_2_REGISTER__VLAN_ENABLE__READ(reg);
    regVals->ethTypeIndex = EMAC_REGS__SCREENING_TYPE_2_REGISTER__INDEX__READ(reg);
    regVals->eTypeEnable  = EMAC_REGS__SCREENING_TYPE_2_REGISTER__ETHERTYPE_ENABLE__READ(reg);
    regVals->compAIndex   = EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_A__READ(reg);
    regVals->compAEnable  = EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_A_ENABLE__READ(reg);
    regVals->compBIndex   = EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_B__READ(reg);
    regVals->compBEnable  = EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_B_ENABLE__READ(reg);
    regVals->compCIndex   = EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_C__READ(reg);
    regVals->compCEnable  = EMAC_REGS__SCREENING_TYPE_2_REGISTER__COMPARE_C_ENABLE__READ(reg);
    return 0;
}
...
uint32_t emacSetType2EthertypeReg(void* pD, uint8_t index, uint16_t eTypeVal) {
    ...
    reg = 0;
    EMAC_REGS__SCREENING_TYPE_2_ETHERTYPE_REG__COMPARE_VALUE__MODIFY(reg, eTypeVal);
    switch (index) {
...
uint32_t emacSetType2CompareReg(void* pD, uint8_t index, CEDI_T2Compare* regVals) {
    ...
    if ((index >= CEDI_PdVar(hwCfg).num_scr2_compare_regs) || (regVals->offsetVal > 0x3F) ||
        (regVals->offsetPosition > CEDI_T2COMP_OFF_TCPUDP) || (regVals->disableMask > 1)) {
        return EINVAL;
    }

    reg0 = 0;
    reg1 = 0;
    EMAC_REGS__TYPE2_COMPARE_WORD_0__MASK_VALUE__MODIFY(reg0, regVals->compMask);
    EMAC_REGS__TYPE2_COMPARE_WORD_0__COMPARE_VALUE__MODIFY(reg0, regVals->compValue);
    EMAC_REGS__TYPE2_COMPARE_WORD_1__OFFSET_VALUE__MODIFY(reg1, regVals->offsetVal);
    EMAC_REGS__TYPE2_COMPARE_WORD_1__COMPARE_OFFSET__MODIFY(reg1, regVals->offsetPosition);
    EMAC_REGS__TYPE2_COMPARE_WORD_1__DISABLE_MASK__MODIFY(reg1, regVals->disableMask);
...
uint32_t emacGetType2CompareReg(void* pD, uint8_t index, CEDI_T2Compare* regVals) {
    ...
    regVals->compMask       = (uint16_t)(EMAC_REGS__TYPE2_COMPARE_WORD_0__MASK_VALUE__READ(reg0));
    regVals->compValue      = (uint16_t)(EMAC_REGS__TYPE2_COMPARE_WORD_0__COMPARE_VALUE__READ(reg0));
    regVals->offsetVal      = (uint8_t)(EMAC_REGS__TYPE2_COMPARE_WORD_1__OFFSET_VALUE__READ(reg1));
    regVals->offsetPosition = (CEDI_T2Offset)(EMAC_REGS__TYPE2_COMPARE_WORD_1__COMPARE_OFFSET__READ(reg1));
    regVals->disableMask    = (uint8_t)(EMAC_REGS__TYPE2_COMPARE_WORD_1__DISABLE_MASK__READ(reg1));
