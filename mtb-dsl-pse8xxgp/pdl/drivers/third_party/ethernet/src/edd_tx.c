...
static void dec_txbd(void* pD, txDesc** bd, uintptr_t** vp, txQueue_t* txQ) {
    if (*bd == txQ->bdBase) {
        *bd = (txDesc*)(((uintptr_t)(*bd)) + (txQ->descMax - 1) * (CEDI_PdVar(txDescriptorSize)));
        *vp += (txQ->descMax - 1);
    }
    else {
        *bd = (txDesc*)(((uintptr_t)(*bd)) - (CEDI_PdVar(txDescriptorSize)));
...
uint32_t emacQueueTxBuf(void* pD, uint8_t queueNum, CEDI_BuffAddr* bufAdd,
    ...
    if ((queueNum >= CEDI_PdVar(cfg).txQs) || (bufAdd->pAddr == 0)) {
        return EINVAL;
    }
    ...
    if (emacTxDescFree(pD, queueNum, &nFree)) {
        return EINVAL;
    }
    ...
    *txQ->vHead = bufAdd->vAddr;
    ...
    if ((flags & CEDI_TXB_LAST_BUFF) && (NULL != bd1stBuf)) {
        ...
        CPS_UncachedWrite32(&bd1stBuf->word[1], CPS_UncachedRead32(&bd1stBuf->word[1]) & ~CEDI_TXD_USED);
        txQ->bd1stBuf = NULL;
    }
...
uint32_t emacQTxBuf(void* pD, CEDI_qTxBufParams* prm) {
    ...
    if ((prm->queueNum >= CEDI_PdVar(cfg).txQs) || (prm->bufAdd->pAddr == 0)) {
        return EINVAL;
    }
    ...
    if (emacTxDescFree(pD, prm->queueNum, &nFree)) {
        return EINVAL;
    }
    ...
    stat |= ((prm->flags & CEDI_TXB_LAST_BUFF) ? CEDI_TXD_LAST_BUF : 0) |
            ((prm->flags & CEDI_TXB_NO_AUTO_CRC) ? CEDI_TXD_NO_AUTO_CRC : 0) | prm->length |
            ((txQ->descNum >= 1) ? ((prm->mssMfs << CEDI_TXD_MSSMFS_SHIFT) & CEDI_TXD_MSSMFS_MASK) : 0);
    ...
    if (!(prm->flags & CEDI_TXB_LAST_BUFF) && (NULL == bd1stBuf)) {
        ...
        stat |= CEDI_TXD_USED
                ...
                | ((prm->flags & CEDI_TXB_TCP_ENCAP) ?
                        /* TSO settings */
                       (CEDI_TXD_TSO_ENABLE | (((prm->tcpStream) << CEDI_TXD_STREAM_SHIFT) & CEDI_TXD_STREAM_MASK) |
...
uint32_t emacDeQTxBuf(void* pD, CEDI_qTxBufParams* prm) {
    ...
    if ((prm->bufAdd == NULL) || (prm->queueNum >= CEDI_PdVar(cfg).txQs)) {
        return EINVAL;
    }
...
uint32_t emacResetTxQ(void* pD, size_t queueNum) {
    #define CEDI_WR_TXQ_PTR_REG_N_CASE(Q)                       \
    case Q :                                                    \
...
#pragma clang optimize on
    ...
    for (size_t i = 0; i < txQ->descMax; i++) {
        ...
        descStartPerQ = (txDesc*)(((uintptr_t)(descStartPerQ)) + (CEDI_PdVar(txDescriptorSize)));
    }
...
uint32_t emacGetTxStatus(void* pD, CEDI_TxStatus* status) {
    ...
    status->txComplete    = EMAC_REGS__TRANSMIT_STATUS__TRANSMIT_COMPLETE__READ(reg);
    status->usedBitRead   = EMAC_REGS__TRANSMIT_STATUS__USED_BIT_READ__READ(reg);
    status->collisionOcc  = EMAC_REGS__TRANSMIT_STATUS__COLLISION_OCCURRED__READ(reg);
    status->retryLimExc   = EMAC_REGS__TRANSMIT_STATUS__RETRY_LIMIT_EXCEEDED__READ(reg);
    status->lateCollision = EMAC_REGS__TRANSMIT_STATUS__LATE_COLLISION_OCCURRED__READ(reg);
    status->txActive      = EMAC_REGS__TRANSMIT_STATUS__TRANSMIT_GO__READ(reg);
    status->txFrameErr    = EMAC_REGS__TRANSMIT_STATUS__AMBA_ERROR__READ(reg);
    status->txUnderRun    = EMAC_REGS__TRANSMIT_STATUS__TRANSMIT_UNDER_RUN__READ(reg);
    status->hRespNotOk    = EMAC_REGS__TRANSMIT_STATUS__RESP_NOT_OK__READ(reg);
...
uint32_t emacSetTxPartialStFwd(void* pD, uint32_t watermark, uint8_t enable) {
    ...
    if ((enable == 1) && ((watermark < 0x14UL) || (watermark >= (1UL << CEDI_PdVar(hwCfg).tx_pbuf_addr)))) {
        return EINVAL;
    }

    reg = CPS_UncachedRead32(CEDI_RegAddr(pbuf_txcutthru));
    if (enable) {
        EMAC_REGS__PBUF_TXCUTTHRU__DMA_TX_CUTTHRU_THRESHOLD__MODIFY(reg, watermark);
...
void emacDisableCbs(void* pD, uint8_t qSel) {
    ...
    if ((qSel > 1) || CEDI_PdVar(hwCfg).exclude_cbs) {
        return;
    }
    reg = CPS_UncachedRead32(CEDI_RegAddr(cbs_control));
    if (qSel) { /* i.e. queue B */
 ...
 * @return ENOTSUP if CBS has been excluded from h/w config
 */
uint32_t emacGetCbsQSetting(void* pD, uint8_t qSel, uint8_t* enable, uint32_t* idleSlope) {
 ...
 * @return 0 if successful.
 */
uint32_t emacSetIpgStretch(void* pD, uint8_t enable, uint8_t multiplier, uint8_t divisor) {
    uint32_t reg;
    ...
    if (enable) {
        ...
        EMAC_REGS__STRETCH_RATIO__IPG_STRETCH__MODIFY(reg, (divisor << 8) | multiplier);
 ...
 */
uint32_t emacGetIpgStretch(void* pD, uint8_t* enabled, uint8_t* multiplier, uint8_t* divisor) {
    uint32_t reg;
    uint32_t stretch;
